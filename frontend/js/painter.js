...
if (uvToggleBtn) {
  uvToggleBtn.onclick = () => {
    ...
    uvToggleBtn.textContent = uvOverlayVisible ? "Hide UV" : "Show UV";
...
// Role classification matching backend texture_gen.py
...
const _UV_ROLE_COLORS = {
  head: "rgba(255, 60, 60, 0.35)",
  face: "rgba(255, 120, 60, 0.35)",
  body: "rgba(60, 120, 255, 0.35)",
  leg:  "rgba(60, 200, 60, 0.35)",
  arm:  "rgba(200, 200, 60, 0.35)",
  tail: "rgba(200, 60, 200, 0.35)",
  ear:  "rgba(60, 200, 200, 0.35)",
};

const _UV_ROLE_BORDERS = {
  head: "rgba(255, 60, 60, 0.8)",
  face: "rgba(255, 120, 60, 0.8)",
  body: "rgba(60, 120, 255, 0.8)",
  leg:  "rgba(60, 200, 60, 0.8)",
  arm:  "rgba(200, 200, 60, 0.8)",
  tail: "rgba(200, 60, 200, 0.8)",
...
// Bedrock box UV layout: compute 6 face rects from uv=[u,v], size=[w,h,d]
...
function _perfaceUVRects(uvDict, cubeSize) {
  const [cw, ch, cd] = cubeSize.length >= 3
    ...
    : [0, 0, 0];
  const defaults = {
    north: [cw, ch], south: [cw, ch],
    east: [cd, ch], west: [cd, ch],
    up: [cw, cd], down: [cw, cd],
  };
  const faces = {};
...
function renderUVOverlay() {
  if (!uvOverlayCanvas) return;
  const ovCtx = uvOverlayCanvas.getContext("2d");

  // Match overlay canvas size to paint canvas
  ...
  // Get geometry from current mob spec
  let geoJson = null;
  try {
  ...
  // Scale factors: pixel coords → canvas coords
  ...
  for (const bone of bones) {
    ...
    for (const cube of (bone.cubes || [])) {
      ...
      if (typeof uv === "object" && !Array.isArray(uv)) {
        ...
        const iw = Math.round(size[0] || 0);
        const ih = Math.round(size[1] || 0);
        const id = Math.round(size[2] || 0);
      ...
      for (const [, [fu, fv, fw, fh]] of Object.entries(faceRects)) {
        ...
        // Label the first face rect of each cube with bone name
...
function setPainterDimensions(w, h) {
  ...
  // Refresh UV overlay if visible
  if (uvOverlayVisible && typeof renderUVOverlay === "function") renderUVOverlay();
}
