...
Usage:
    ...
    python scripts/evaluate_llm.py --provider openai         # GPT-5.2 via GitHub Models
...
# ---------------------------------------------------------------------------
# Context injection — shared with backend/llm/category_context.py
# ---------------------------------------------------------------------------
sys.path.insert(0, str(PROJECT_ROOT))

from backend.llm.category_context import (
    CATEGORY_CONTEXT, CATEGORY_SCHEMAS as _CATEGORY_SCHEMAS,
    VANILLA_REF as _VANILLA_REF,
)
...
# CATEGORY_CONTEXT is imported from backend.llm.category_context
...
def _call_openai(test_case: dict, model: str, strategy: str = "few_shot") -> dict:
    """Call OpenAI-compatible API via GitHub Models."""
    if _OpenAI is None:
        raise RuntimeError("openai package not installed. Run: pip install openai")
    key = os.environ.get("GITHUB_TOKEN")
    if not key:
        raise RuntimeError("GITHUB_TOKEN environment variable not set. Add it to your .env file.")
    system_prompt = _build_system_prompt(test_case.get("category", ""), strategy=strategy)
    client = _OpenAI(
