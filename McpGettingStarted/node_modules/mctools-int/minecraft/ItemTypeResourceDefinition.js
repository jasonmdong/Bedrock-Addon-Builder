"use strict";
var __create = Object.create;
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __getProtoOf = Object.getPrototypeOf;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toESM = (mod, isNodeMode, target) => (target = mod != null ? __create(__getProtoOf(mod)) : {}, __copyProps(
  // If the importer is in node compatibility mode or this is not an ESM
  // file that has been converted to a CommonJS file using a Babel-
  // compatible transform (i.e. "__esModule" has not been set), then set
  // "default" to the CommonJS "module.exports" for node compatibility.
  isNodeMode || !mod || !mod.__esModule ? __defProp(target, "default", { value: mod, enumerable: true }) : target,
  mod
));
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var ItemTypeResourceDefinition_exports = {};
__export(ItemTypeResourceDefinition_exports, {
  default: () => ItemTypeResourceDefinition
});
module.exports = __toCommonJS(ItemTypeResourceDefinition_exports);
var import_Log = __toESM(require("../core/Log"));
var import_ste_events = require("ste-events");
var import_ManagedComponent = require("./ManagedComponent");
var import_StorageUtilities = __toESM(require("../storage/StorageUtilities"));
var import_Database = __toESM(require("./Database"));
var import_MinecraftUtilities = __toESM(require("./MinecraftUtilities"));
var import_Utilities = __toESM(require("../core/Utilities"));
class ItemTypeResourceDefinition {
  wrapper = null;
  _behaviorPackFile;
  _id;
  _isLoaded = false;
  _loadedWithComments = false;
  _managed = {};
  _data;
  _onLoaded = new import_ste_events.EventDispatcher();
  _onComponentAdded = new import_ste_events.EventDispatcher();
  _onComponentRemoved = new import_ste_events.EventDispatcher();
  _onComponentChanged = new import_ste_events.EventDispatcher();
  get data() {
    return this._data;
  }
  get onComponentAdded() {
    return this._onComponentAdded.asEvent();
  }
  get onComponentRemoved() {
    return this._onComponentRemoved.asEvent();
  }
  get onComponentChanged() {
    return this._onComponentChanged.asEvent();
  }
  get isLoaded() {
    return this._isLoaded;
  }
  get behaviorPackFile() {
    return this._behaviorPackFile;
  }
  get onLoaded() {
    return this._onLoaded.asEvent();
  }
  set behaviorPackFile(newFile) {
    this._behaviorPackFile = newFile;
  }
  get id() {
    if (this._id === void 0) {
      return "";
    }
    return this._id;
  }
  set id(newId) {
    this._id = newId;
  }
  get shortId() {
    if (this._id !== void 0) {
      if (this._id.startsWith("minecraft:")) {
        return this._id.substring(10, this._id.length);
      }
      return this._id;
    }
    return void 0;
  }
  async getFormatVersionIsCurrent() {
    const fv = this.getFormatVersion();
    if (fv === void 0 || fv.length !== 3) {
      return false;
    }
    return await import_Database.default.isRecentVersionFromVersionArray(fv);
  }
  getFormatVersion() {
    if (!this.wrapper) {
      return void 0;
    }
    return import_MinecraftUtilities.default.getVersionArrayFrom(this.wrapper.format_version);
  }
  getComponent(id) {
    if (this._data === void 0) {
      return void 0;
    }
    if (!this._managed[id]) {
      const comp = this._data.components[id];
      if (comp) {
        this._managed[id] = new import_ManagedComponent.ManagedComponent(this._data.components, id, comp);
      }
    }
    return this._managed[id];
  }
  notifyComponentUpdated(id) {
    const component = this.getComponent(id);
    if (component === void 0) {
      import_Log.default.unexpectedUndefined("ITRNCU");
    } else {
      this._onComponentChanged.dispatch(this, component);
    }
  }
  getAllComponents() {
    return this.getComponents();
  }
  getComponents() {
    const componentSet = [];
    if (this._data !== void 0) {
      for (const componentName in this._data.components) {
        const component = this.getComponent(componentName);
        if (component !== void 0) {
          componentSet.push(component);
        }
      }
    }
    return componentSet;
  }
  setResourcePackFormatVersion(versionStr) {
    this._ensureDataInitialized();
    if (this.wrapper) {
      this.wrapper.format_version = versionStr;
    }
  }
  addComponent(id, componentOrData) {
    this._ensureDataInitialized();
    const bpData = this._data;
    const mc = componentOrData instanceof import_ManagedComponent.ManagedComponent ? componentOrData : new import_ManagedComponent.ManagedComponent(bpData.components, id, componentOrData);
    bpData.components[id] = mc.getData();
    this._managed[id] = mc;
    this._onComponentAdded.dispatch(this, mc);
    return mc;
  }
  removeComponent(id) {
    if (this._data === void 0) {
      return;
    }
    const newBehaviorPacks = {};
    const newManagedComponents = {};
    for (const name in this._data.components) {
      if (name !== id && import_Utilities.default.isUsableAsObjectKey(name)) {
        const component = this._data.components[name];
        newBehaviorPacks[name] = component;
      }
    }
    for (const name in this._managed) {
      if (name !== id && import_Utilities.default.isUsableAsObjectKey(name)) {
        newManagedComponents[name] = this._managed[name];
      }
    }
    this._data.components = newBehaviorPacks;
    this._managed = newManagedComponents;
  }
  _ensureDataInitialized() {
    if (this._data === void 0) {
      this._data = {
        description: {
          identifier: "unknown"
        },
        components: {}
      };
    }
  }
  static async ensureOnFile(file, loadHandler) {
    let itt;
    if (file.manager === void 0) {
      itt = new ItemTypeResourceDefinition();
      itt.behaviorPackFile = file;
      file.manager = itt;
    }
    if (file.manager !== void 0 && file.manager instanceof ItemTypeResourceDefinition) {
      itt = file.manager;
      if (!itt.isLoaded) {
        if (loadHandler) {
          itt.onLoaded.subscribe(loadHandler);
        }
        await itt.load();
      }
    }
    return itt;
  }
  persist() {
    if (this._behaviorPackFile === void 0) {
      return false;
    }
    if (!this.wrapper) {
      import_Log.default.unexpectedUndefined("ITRDP");
      return false;
    }
    return this._behaviorPackFile.setObjectContentIfSemanticallyDifferent(this.wrapper);
  }
  /**
   * Loads the definition from the file.
   * @param preserveComments If true, uses comment-preserving JSON parsing for edit/save cycles.
   *                         If false (default), uses efficient standard JSON parsing.
   *                         Can be called again with true to "upgrade" a read-only load to read/write.
   */
  async load(preserveComments = false) {
    if (this._isLoaded && this._loadedWithComments) {
      return;
    }
    if (this._isLoaded && !preserveComments) {
      return;
    }
    if (this._behaviorPackFile === void 0) {
      return;
    }
    if (!this._behaviorPackFile.isContentLoaded) {
      await this._behaviorPackFile.loadContent();
    }
    if (this._behaviorPackFile.content === null || this._behaviorPackFile.content instanceof Uint8Array) {
      this._isLoaded = true;
      this._loadedWithComments = preserveComments;
      this._onLoaded.dispatch(this, this);
      return;
    }
    this.wrapper = preserveComments ? import_StorageUtilities.default.getJsonObjectWithComments(this._behaviorPackFile) : import_StorageUtilities.default.getJsonObject(this._behaviorPackFile);
    if (this.wrapper) {
      const item = this.wrapper["minecraft:item"];
      if (item && item.description) {
        this.id = item.description.identifier;
      }
      this._data = item;
    }
    this._isLoaded = true;
    this._loadedWithComments = preserveComments;
    this._onLoaded.dispatch(this, this);
  }
}
//# sourceMappingURL=ItemTypeResourceDefinition.js.map
