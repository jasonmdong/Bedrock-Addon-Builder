...
var BiomeResourceDefinition_exports = {};
__export(BiomeResourceDefinition_exports, {
  default: () => BiomeResourceDefinition
});
module.exports = __toCommonJS(BiomeResourceDefinition_exports);
...
class BiomeResourceDefinition {
  ...
  get clientBiomeData() {
    ...
    return this._data["minecraft:client_biome"];
  }
  get isLoaded() {
  ...
  getComponent(id) {
    ...
    if (!this._managed[id]) {
      if (this._data["minecraft:client_biome"]?.components) {
        const comp = this._data["minecraft:client_biome"].components[id];
        if (comp) {
          this._managed[id] = new import_ManagedComponent.ManagedComponent(
            this._data["minecraft:client_biome"].components,
            id,
            comp
          );
  ...
  notifyComponentUpdated(id) {
    ...
    if (component === void 0) {
      import_Log.default.unexpectedUndefined("BDNCU");
    } else {
  ...
  getComponents() {
    const componentSet = [];
    if (this._data !== void 0 && this._data["minecraft:client_biome"]?.components) {
  ...
  addComponent(id, componentOrData) {
    ...
    const mc = componentOrData instanceof import_ManagedComponent.ManagedComponent ? componentOrData : new import_ManagedComponent.ManagedComponent(
      biomeData["minecraft:client_biome"].components,
      id,
      componentOrData
    );
    biomeData["minecraft:client_biome"].components[id] = mc.getData();
    this._managed[id] = mc;
    ...
    for (const name in this._data["minecraft:client_biome"].components) {
      if (name !== id) {
        if (import_Utilities.default.isUsableAsObjectKey(name)) {
          const component = this._data["minecraft:client_biome"].components[name];
  ...
  async load() {
    ...
    try {
      ...
      import_Log.default.error("Could not load biome definition: " + e);
    }
  }
  get shortId() {
...
//# sourceMappingURL=BiomeResourceDefinition.js.map
