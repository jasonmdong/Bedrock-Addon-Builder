...
class Converter {
  static cubeEnsureBedrockProperties(cube) {
  ...
  static blockEnsureBedrockProperties(block) {
    for (const propName in block.properties) {
      const prop = block.getProperty(propName);
      switch (propName) {
        case "facing":
          if (prop.value === "east") {
            block.ensureProperty("facing_direction").value = import_Block.BlockFacingDirection.east;
          } else if (prop.value === "west") {
            block.ensureProperty("facing_direction").value = import_Block.BlockFacingDirection.west;
          } else if (prop.value === "north") {
            block.ensureProperty("facing_direction").value = import_Block.BlockFacingDirection.north;
          } else if (prop.value === "south") {
            block.ensureProperty("facing_direction").value = import_Block.BlockFacingDirection.south;
          }
          break;
        ...
        case "axis":
          ...
          if (block.shortTypeId && block.shortTypeId.indexOf("nether") >= 0) {
            ...
            if (sourceAxisVal === "x") {
              ...
              block.ensureProperty("facing_direction").value = import_Block.BlockFacingDirection.up;
            } else if (sourceAxisVal === "z") {
              block.ensureProperty("facing_direction").value = import_Block.BlockFacingDirection.east;
            }
          }
        ...
        case "waterlogged":
          if (prop.asBoolean(false) === true) {
            block.extraLiquidDepth = import_Block.default.MAX_WATER_LEVEL;
          } else {
        ...
        default:
          import_Log.default.debugAlert(
            "Unsupported property found: " + propName + " with value '" + prop.value + "' at block " + block.x + " " + block.y + " " + block.z
          );
          break;
      }
    }
