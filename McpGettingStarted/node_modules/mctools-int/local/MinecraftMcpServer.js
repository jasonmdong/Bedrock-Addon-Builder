"use strict";
var __create = Object.create;
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __getProtoOf = Object.getPrototypeOf;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toESM = (mod, isNodeMode, target) => (target = mod != null ? __create(__getProtoOf(mod)) : {}, __copyProps(
  // If the importer is in node compatibility mode or this is not an ESM
  // file that has been converted to a CommonJS file using a Babel-
  // compatible transform (i.e. "__esModule" has not been set), then set
  // "default" to the CommonJS "module.exports" for node compatibility.
  isNodeMode || !mod || !mod.__esModule ? __defProp(target, "default", { value: mod, enumerable: true }) : target,
  mod
));
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);
var MinecraftMcpServer_exports = {};
__export(MinecraftMcpServer_exports, {
  default: () => MinecraftMcpServer
});
module.exports = __toCommonJS(MinecraftMcpServer_exports);
var import_mcp = require("@modelcontextprotocol/sdk/server/mcp.js");
var import_stdio = require("@modelcontextprotocol/sdk/server/stdio.js");
var import_zod = require("zod");
var import_streamableHttp = require("@modelcontextprotocol/sdk/server/streamableHttp.js");
var import_types = require("@modelcontextprotocol/sdk/types.js");
var import_crypto = require("crypto");
var import_Log = __toESM(require("../core/Log"));
var import_HttpUtilities = __toESM(require("./HttpUtilities"));
var import_Database = __toESM(require("../minecraft/Database"));
var import_DataFormZod = __toESM(require("../dataform/DataFormZod"));
var import_ServerManager = __toESM(require("./ServerManager"));
var import_ICreatorToolsData = require("../app/ICreatorToolsData");
var import_WorldLevelDat = require("../minecraft/WorldLevelDat");
var import_Status = require("../app/Status");
var import_Package = __toESM(require("../app/Package"));
var import_Utilities = __toESM(require("../core/Utilities"));
var import_Project = __toESM(require("../app/Project"));
var import_ProjectExporter = __toESM(require("../app/ProjectExporter"));
var import_ProjectUtilities = __toESM(require("../app/ProjectUtilities"));
var import_ProjectItemCreateManager = __toESM(require("../app/ProjectItemCreateManager"));
var import_ClUtils = __toESM(require("../cli/ClUtils"));
var import_CreatorToolsHost = __toESM(require("../app/CreatorToolsHost"));
var import_StorageUtilities = __toESM(require("../storage/StorageUtilities"));
var import_ModelDesignUtilities = __toESM(require("../minecraft/ModelDesignUtilities"));
var import_ModelDesignTemplates = require("../minecraft/ModelDesignTemplates");
var import_StructureUtilities = __toESM(require("../minecraft/StructureUtilities"));
var import_ContentMetaSchemaZod = require("../minecraft/ContentMetaSchemaZod");
var import_ContentGenerator = require("../minecraft/ContentGenerator");
var import_ContentSchemaInferrer = __toESM(require("../minecraft/ContentSchemaInferrer"));
var import_PlaywrightPageRenderer = __toESM(require("./PlaywrightPageRenderer"));
var import_ImageGenerationUtilities = __toESM(require("./ImageGenerationUtilities"));
var import_ServerManager2 = require("./ServerManager");
var import_NodeStorage = __toESM(require("./NodeStorage"));
var import_ModelDesignDefinition = __toESM(require("../design/ModelDesignDefinition"));
var import_StructureDesignDefinition = __toESM(require("../design/StructureDesignDefinition"));
var import_IProjectItemData = require("../app/IProjectItemData");
var fs = __toESM(require("fs"));
var path = __toESM(require("path"));
var net = __toESM(require("net"));
var import_pngjs = require("pngjs");
var import_LocalUtilities = require("./LocalUtilities");
class MinecraftMcpServer {
  /** Starting port for the internal HTTP server range */
  static PORT_RANGE_START = 6136;
  /** Ending port for the internal HTTP server range (200 ports available) */
  static PORT_RANGE_END = 6336;
  /** Maximum attempts to find an available port before giving up */
  static PORT_MAX_ATTEMPTS = 20;
  _server;
  _env = void 0;
  _transports = {};
  _creatorTools = void 0;
  _serverManager;
  /** Cache for loaded MCP preferences, keyed by folder path where prefs.json was found */
  _mcpPrefsCache = {};
  /** Folders we've already checked and found no prefs.json (negative cache) */
  _mcpPrefsNotFoundFolders = /* @__PURE__ */ new Set();
  /** HTTP server port for model preview rendering (dynamically assigned on startup) */
  _previewServerPort = MinecraftMcpServer.PORT_RANGE_START;
  /** Cached PlaywrightPageRenderer for reuse across preview operations */
  _cachedRenderer;
  /** Flag to prevent multiple cleanup calls */
  _cleaningUp = false;
  /**
   * Working folder path for MCP operations.
   * When set via the -i argument, this folder is used as the default context for
   * file operations and is exposed to AI assistants via the MCP protocol's prompts.
   */
  _workingFolder = void 0;
  /** Getter for the working folder */
  get workingFolder() {
    return this._workingFolder;
  }
  constructor() {
    this._server = new import_mcp.McpServer({
      name: "minecraft-creator-tools",
      version: "1.0.0"
    });
    this._processValidateContent = this._processValidateContent.bind(this);
    this._processValidateContentAtPath = this._processValidateContentAtPath.bind(this);
    this._runActionSet = this._runActionSet.bind(this);
    this._moveSessionPlayerToLocation = this._moveSessionPlayerToLocation.bind(this);
    this._sessionOp = this._sessionOp.bind(this);
    this._runActionSetOp = this._runActionSetOp.bind(this);
    this._createOp = this._createOp.bind(this);
    this._addOp = this._addOp.bind(this);
    this._createMinecraftSession = this._createMinecraftSession.bind(this);
    this._runCommandOp = this._runCommandOp.bind(this);
    this._readImageFileOp = this._readImageFileOp.bind(this);
    this._writeImageFileFromBase64Op = this._writeImageFileFromBase64Op.bind(this);
    this._writeImageFileFromSvgOp = this._writeImageFileFromSvgOp.bind(this);
    this._writeImageFileFromPixelArtOp = this._writeImageFileFromPixelArtOp.bind(this);
    this._designModelOp = this._designModelOp.bind(this);
    this._designStructureOp = this._designStructureOp.bind(this);
    this._createMinecraftContentOp = this._createMinecraftContentOp.bind(this);
    this._getEffectiveContentSchemaOp = this._getEffectiveContentSchemaOp.bind(this);
  }
  /**
   * Ensures a ServerManager instance is available, creating one if needed.
   * Always sets the "mcp" slot prefix to isolate MCP servers from other contexts.
   */
  ensureServerManager() {
    if (!this._env || !this._creatorTools) {
      throw new Error("Creator Tools is not initialized");
    }
    if (!this._serverManager) {
      this._serverManager = new import_ServerManager.default(this._env, this._creatorTools);
      this._serverManager.slotPrefix = "mcp";
    }
    return this._serverManager;
  }
  async handleRequest(req, res) {
    const sessionId = req.headers["mcp-session-id"];
    if (req.method === "POST") {
      const body = [];
      req.on("data", (chunk) => {
        body.push(Buffer.from(chunk, "binary"));
      });
      req.on("end", async () => {
        if (body.length >= 1) {
          const bodyContent = Buffer.concat(body);
          const sessionId2 = req.headers["mcp-session-id"];
          let transport = void 0;
          if (sessionId2 && this._transports[sessionId2]) {
            transport = this._transports[sessionId2];
          } else if (!sessionId2 && (0, import_types.isInitializeRequest)(bodyContent)) {
            transport = new import_streamableHttp.StreamableHTTPServerTransport({
              sessionIdGenerator: () => (0, import_crypto.randomUUID)(),
              onsessioninitialized: (sessionId3) => {
                if (transport) {
                  this._transports[sessionId3] = transport;
                }
              },
              // DNS rebinding protection is disabled by default for backwards compatibility. If you are running this server
              // locally, make sure to set:
              enableDnsRebindingProtection: true,
              allowedHosts: ["127.0.0.1"]
            });
            transport.onclose = () => {
              if (transport) {
                if (transport.sessionId) {
                  delete this._transports[transport.sessionId];
                }
              }
            };
            await this._server.connect(transport);
          }
          if (transport) {
            await transport.handleRequest(req, res, bodyContent);
          }
        }
      });
    } else if (req.method === "GET" || req.method === "DELETE") {
      const sessionId2 = req.headers["mcp-session-id"];
      if (!sessionId2 || !this._transports[sessionId2]) {
        this.sendErrorRequest(400, "Invalid or missing session ID", req, res);
        return;
      }
      const transport = this._transports[sessionId2];
      await transport.handleRequest(req, res);
    }
  }
  sendErrorRequest(statusCode, message, req, res) {
    import_Log.default.message(import_HttpUtilities.default.getShortReqDescription(req) + "Error request: " + message);
    if (!res.headersSent) {
      res.writeHead(statusCode);
    }
    res.end(message);
  }
  /**
   * Finds MCP preferences for a given file path by looking for .mct/mcp/prefs.json
   * in the file's parent folder and up to 11 levels of parent folders.
   * Results are cached per session to avoid redundant file system reads.
   *
   * @param filePath The absolute path to the file being accessed
   * @returns The MCP preferences if found, or undefined if no prefs.json exists`
   */
  getMcpPrefsForPath(filePath) {
    const normalizedPath = path.normalize(filePath);
    let currentDir = path.dirname(normalizedPath);
    for (let i = 0; i < 10; i++) {
      if (this._mcpPrefsCache[currentDir]) {
        return this._mcpPrefsCache[currentDir];
      }
      if (this._mcpPrefsNotFoundFolders.has(currentDir)) {
        const parentDir2 = path.dirname(currentDir);
        if (parentDir2 === currentDir) {
          break;
        }
        currentDir = parentDir2;
        continue;
      }
      const prefsPath = path.join(currentDir, ".mct", "mcp", "prefs.json");
      if (fs.existsSync(prefsPath)) {
        try {
          const prefsContent = fs.readFileSync(prefsPath, "utf-8");
          const prefs = JSON.parse(prefsContent);
          this._mcpPrefsCache[currentDir] = prefs;
          return prefs;
        } catch (error) {
          import_Log.default.debugAlert(`Failed to parse MCP prefs at ${prefsPath}: ${error}`);
          this._mcpPrefsNotFoundFolders.add(currentDir);
        }
      } else {
        this._mcpPrefsNotFoundFolders.add(currentDir);
      }
      const parentDir = path.dirname(currentDir);
      if (parentDir === currentDir) {
        break;
      }
      currentDir = parentDir;
    }
    return void 0;
  }
  /**
   * Checks if a specific MCP preference flag is enabled for a given file path.
   *
   * @param filePath The absolute path to the file being accessed
   * @param prefKey The preference key to check
   * @returns true if the preference is explicitly set to true, false otherwise
   */
  isMcpPrefEnabled(filePath, prefKey) {
    const prefs = this.getMcpPrefsForPath(filePath);
    if (!prefs) {
      return false;
    }
    return prefs[prefKey] === true;
  }
  async _processValidateContent(args) {
    if (!this._creatorTools) {
      throw new Error("Creator Tools is not initialized");
    }
    const projectOrError = await this._creatorTools.createProjectFromContent(args.jsonContentOrBase64ZipContent);
    if (!projectOrError || typeof projectOrError === "string") {
      throw new Error(
        "Failed to create project. Was the content a valid Base64-encoded ZIP file?" + (typeof projectOrError === "string" ? " Error: " + projectOrError : "")
      );
    }
    const pis = projectOrError.indevInfoSet;
    await pis.generateForProject();
    const resultObject = pis.getDataObject(void 0, void 0, void 0);
    return {
      content: [{ type: "text", text: JSON.stringify(resultObject, null, 2) }],
      structuredContent: { info: resultObject }
    };
  }
  async _sessionOp(args) {
    return {
      content: [{ type: "text", text: "Successfully completed" }]
    };
  }
  async _moveSessionPlayerToLocation(args) {
    let result = await this._runActionSet({
      name: "Test Player Move Action Set",
      targetType: 1,
      actions: [
        {
          type: "test_simulated_player_move",
          name: args.playerName,
          location: [
            args.locationToHavePlayerMoveTo.x,
            args.locationToHavePlayerMoveTo.y,
            args.locationToHavePlayerMoveTo.z
          ]
        }
      ]
    });
    return {
      content: [{ type: "text", text: "Successfully started session with content" }],
      structuredContent: { state: result }
    };
  }
  async _createMinecraftSession(args) {
    if (!this._creatorTools || !this._env) {
      throw new Error("Creator Tools is not initialized");
    }
    const serverManager = this.ensureServerManager();
    await this._env.load();
    await serverManager.prepare();
    serverManager.ensureHttpServer(6128);
    const startMessage = {
      mode: import_ICreatorToolsData.DedicatedServerMode.auto,
      path: void 0,
      additionalContentPath: args.packagedMcaddonOrMcworldFilePath,
      forceStartNewWorld: true,
      // Always start with a fresh world for MCP sessions
      worldSettings: this.getWorldSettings(),
      transientWorld: true
      // Mark as transient - world data is reset each deployment
    };
    let srvr = await serverManager.ensureActiveServer(0, startMessage);
    if (srvr) {
      await srvr.startServer(false, startMessage);
      await srvr.waitUntilStarted();
    }
    let result = await this._runActionSet({
      name: "Test Player Spawn Action Set",
      targetType: 1,
      actions: [{ type: "test_simulated_player_spawn", name: args.testPlayerNameToUse, location: [0, 0, 0] }]
    });
    return {
      content: [{ type: "text", text: "Successfully started session with content" }],
      structuredContent: { state: result }
    };
  }
  async _runActionSet(actionSet) {
    let actionSetStr = JSON.stringify(actionSet);
    actionSetStr = actionSetStr.replace(/\"/g, "|");
    let token = import_Utilities.default.createRandomLowerId(6);
    let result = await this.runCommand('scriptevent mct:actionset "' + token + "|" + actionSetStr + '"', token + "|");
    if (result) {
      let rasIndex = result.indexOf("ras|");
      if (rasIndex) {
        let nextPipe = result.indexOf("|", rasIndex + 5);
        if (nextPipe >= 0) {
          const resultJsonStr = result.substring(nextPipe + 1);
          try {
            const resultObject = JSON.parse(resultJsonStr);
            return resultObject;
          } catch (e) {
            return void 0;
          }
        }
      }
    }
    return void 0;
  }
  getWorldSettings() {
    let gt = import_WorldLevelDat.GameType.survival;
    let generator = import_WorldLevelDat.Generator.infinite;
    let difficulty = import_WorldLevelDat.Difficulty.easy;
    let randomSeed = "2000";
    const packRefs = [];
    import_Package.default.ensureMinecraftCreatorToolsPackageReference(packRefs);
    const worldSettings = {
      gameType: gt,
      generator,
      cheatsEnabled: true,
      difficulty,
      playerPermissionLevel: import_WorldLevelDat.PlayerPermissionsLevel.operator,
      permissionLevel: import_WorldLevelDat.PlayerPermissionsLevel.operator,
      randomSeed,
      packageReferences: packRefs,
      worldTemplateReferences: void 0
    };
    return worldSettings;
  }
  async _runActionSetOp(args) {
    if (!this._creatorTools || !this._env || !args.actionSet) {
      throw new Error("Creator Tools is not initialized");
    }
    const serverManager = this.ensureServerManager();
    let srvr = await serverManager.ensureActiveServer(0);
    if (srvr) {
      this._runActionSet(args.actionSet);
    }
    return {
      content: [{ type: "text", text: "Successfully ran action set operation" }]
    };
  }
  async _runCommandOp(args) {
    let result = await this.runCommand(args.command);
    return {
      content: [{ type: "text", text: result ? result : "No result from command" }]
    };
  }
  async runCommand(command, token) {
    if (!this._creatorTools || !this._env) {
      throw new Error("Creator Tools is not initialized");
    }
    const serverManager = this.ensureServerManager();
    let srvr = await serverManager.ensureActiveServer(0);
    let result = void 0;
    if (srvr) {
      result = await srvr.runCommandImmediate(command, token);
    }
    return result;
  }
  async _processValidateContentAtPath(args) {
    if (!this._creatorTools) {
      throw new Error("Creator Tools is not initialized");
    }
    const projectOrError = await this._creatorTools.createProjectFromPath(args.filePath);
    if (!projectOrError || typeof projectOrError === "string") {
      throw new Error(
        "Failed to create project. Was the content a valid Base64-encoded ZIP file?" + (typeof projectOrError === "string" ? " Error: " + projectOrError : "")
      );
    }
    const pis = projectOrError.indevInfoSet;
    await pis.generateForProject();
    const resultObject = pis.getDataObject(void 0, void 0, void 0);
    return {
      content: [{ type: "text", text: JSON.stringify(resultObject, null, 2) }],
      structuredContent: { info: resultObject }
    };
  }
  async _create(project, title, description, newName, creator, template) {
    if (!this._env || !this._creatorTools) {
      return;
    }
    await this._env.load();
    if (!this._env.iAgreeToTheMinecraftEndUserLicenseAgreementAndPrivacyStatementAtMinecraftDotNetSlashEula) {
      import_Log.default.message("The Minecraft End User License Agreement and Privacy Statement was not agreed to.");
      return;
    }
    await this._creatorTools.loadGallery();
    if (!this._creatorTools.gallery) {
      import_Log.default.message("Not configured correctly to create a project (no gallery).");
      return;
    }
    const galProjects = this._creatorTools.gallery.items;
    let galProject;
    if (template) {
      for (let i = 0; i < galProjects.length; i++) {
        const galProjectCand = galProjects[i];
        if (galProjectCand && galProjectCand.id && galProjectCand.id.toLowerCase() === template.toLowerCase()) {
          galProject = galProjectCand;
        }
      }
    }
    if (!newName) {
      import_Log.default.error("Not configured correctly to create a project.");
      return;
    }
    if (!galProject) {
      import_Log.default.error("No project was selected.");
      return;
    }
    project = await import_ProjectExporter.default.syncProjectFromGitHub(
      true,
      this._creatorTools,
      galProject.gitHubRepoName,
      galProject.gitHubOwner,
      galProject.gitHubBranch,
      galProject.gitHubFolder,
      newName,
      project,
      galProject.fileList,
      async (message) => {
        import_Log.default.message(message);
      },
      true
    );
    let suggestedShortName = void 0;
    if (newName && creator) {
      suggestedShortName = import_ProjectUtilities.default.getSuggestedProjectShortName(creator, newName);
    }
    if (creator) {
      await import_ProjectUtilities.default.applyCreator(project, creator);
    }
    await import_ProjectUtilities.default.processNewProject(project, title, description, suggestedShortName, false);
  }
  async _add(project, templateType, newName) {
    if (!this._env || !this._creatorTools) {
      import_Log.default.error("Not configured correctly to create a project (no mctools core).");
      return false;
    }
    await this._env.load();
    if (!this._env.iAgreeToTheMinecraftEndUserLicenseAgreementAndPrivacyStatementAtMinecraftDotNetSlashEula) {
      import_Log.default.message("The Minecraft End User License Agreement and Privacy Statement was not agreed to.");
      return false;
    }
    await this._creatorTools.loadGallery();
    if (!this._creatorTools.gallery) {
      import_Log.default.message("Not configured correctly to add an item (no gallery).");
      return false;
    }
    if (templateType && newName) {
      for (const galItem of this._creatorTools.gallery.items) {
        if (galItem.id === templateType) {
          await import_ProjectItemCreateManager.default.addFromGallery(project, newName, galItem);
          await project.save();
          return true;
        }
      }
    }
    return false;
  }
  async _createOp(args) {
    if (!this._creatorTools) {
      throw new Error("Creator Tools is not initialized");
    }
    if (!fs.existsSync(args.folderPathToCreateProjectAt)) {
      fs.mkdirSync(args.folderPathToCreateProjectAt, { recursive: true });
    }
    const project = import_ClUtils.default.createProject(this._creatorTools, {
      ctorProjectName: args.newName,
      localFolderPath: args.folderPathToCreateProjectAt
    });
    await this._create(
      project,
      args.title,
      args.description ? args.description : "",
      args.newName,
      args.creator,
      args.template
    );
    return {
      content: [{ type: "text", text: "Created! Additional files were added to your project." }]
    };
  }
  async _addOp(args) {
    if (!this._creatorTools) {
      throw new Error("Creator Tools is not initialized");
    }
    if (!fs.existsSync(args.folderPathToCreateProjectAt)) {
      fs.mkdirSync(args.folderPathToCreateProjectAt, { recursive: true });
    }
    const project = import_ClUtils.default.createProject(this._creatorTools, {
      ctorProjectName: args.name,
      localFolderPath: args.folderPathToCreateProjectAt
    });
    let result = await this._add(project, args.templateType, args.name);
    if (!result) {
      return {
        content: [{ type: "text", text: "No items were added." }]
      };
    } else {
      return {
        content: [{ type: "text", text: "Additional items were added." }]
      };
    }
  }
  /**
   * Creates Minecraft content from a meta-schema definition.
   * This is a simplified, AI-friendly format that generates all required files.
   */
  async _createMinecraftContentOp(args) {
    if (!this._creatorTools) {
      throw new Error("Creator Tools is not initialized");
    }
    try {
      const parseResult = import_ContentMetaSchemaZod.MinecraftContentSchema.safeParse(args.definition);
      if (!parseResult.success) {
        return {
          content: [
            {
              type: "text",
              text: `Validation error: ${parseResult.error.errors.map((e) => `${e.path.join(".")}: ${e.message}`).join(", ")}`
            }
          ]
        };
      }
      const generator = new import_ContentGenerator.ContentGenerator(parseResult.data);
      const generated = await generator.generate();
      if (!fs.existsSync(args.outputPath)) {
        fs.mkdirSync(args.outputPath, { recursive: true });
      }
      const filesWritten = [];
      const writeFile = (file) => {
        let basePath = args.outputPath;
        if (file.pack === "behavior") {
          basePath = path.join(args.outputPath, "behavior_packs", parseResult.data.namespace || "custom");
        } else if (file.pack === "resource") {
          basePath = path.join(args.outputPath, "resource_packs", parseResult.data.namespace || "custom");
        }
        const fullPath = path.join(basePath, file.path);
        const dirPath = path.dirname(fullPath);
        if (!fs.existsSync(dirPath)) {
          fs.mkdirSync(dirPath, { recursive: true });
        }
        if (file.type === "json") {
          fs.writeFileSync(fullPath, JSON.stringify(file.content, null, 2), "utf-8");
        } else if (file.type === "text") {
          fs.writeFileSync(fullPath, file.content, "utf-8");
        } else if (file.type === "png") {
          if (file.content instanceof Uint8Array) {
            fs.writeFileSync(fullPath, Buffer.from(file.content));
          } else {
            fs.writeFileSync(fullPath, Buffer.from(file.content, "base64"));
          }
        }
        filesWritten.push(fullPath);
      };
      if (generated.behaviorPackManifest) writeFile(generated.behaviorPackManifest);
      if (generated.resourcePackManifest) writeFile(generated.resourcePackManifest);
      for (const file of generated.entityBehaviors) writeFile(file);
      for (const file of generated.entityResources) writeFile(file);
      for (const file of generated.blockBehaviors) writeFile(file);
      for (const file of generated.blockResources) writeFile(file);
      for (const file of generated.itemBehaviors) writeFile(file);
      for (const file of generated.itemResources) writeFile(file);
      for (const file of generated.lootTables) writeFile(file);
      for (const file of generated.recipes) writeFile(file);
      for (const file of generated.spawnRules) writeFile(file);
      for (const file of generated.features) writeFile(file);
      for (const file of generated.featureRules) writeFile(file);
      for (const file of generated.textures) writeFile(file);
      for (const file of generated.geometries) writeFile(file);
      for (const file of generated.renderControllers) writeFile(file);
      if (generated.terrainTextures) writeFile(generated.terrainTextures);
      if (generated.itemTextures) writeFile(generated.itemTextures);
      const summary = generated.summary;
      let summaryText = `Generated ${filesWritten.length} files for namespace "${summary.namespace}":
`;
      summaryText += `- ${summary.entityCount} entity types
`;
      summaryText += `- ${summary.blockCount} block types
`;
      summaryText += `- ${summary.itemCount} item types
`;
      summaryText += `- ${summary.lootTableCount} loot tables
`;
      summaryText += `- ${summary.recipeCount} recipes
`;
      summaryText += `- ${summary.spawnRuleCount} spawn rules
`;
      summaryText += `- ${summary.featureCount} features
`;
      if (summary.warnings.length > 0) {
        summaryText += `
Warnings:
${summary.warnings.map((w) => `- ${w}`).join("\n")}`;
      }
      if (summary.errors.length > 0) {
        summaryText += `
Errors:
${summary.errors.map((e) => `- ${e}`).join("\n")}`;
      }
      return {
        content: [{ type: "text", text: summaryText }],
        structuredContent: {
          filesWritten,
          summary: generated.summary
        }
      };
    } catch (error) {
      return {
        content: [{ type: "text", text: `Error generating content: ${error}` }]
      };
    }
  }
  /**
   * Gets the effective content schema for an existing Minecraft project.
   * This analyzes the project's entities, blocks, items, etc. and infers what
   * traits and simplified properties would represent them in meta-schema format.
   *
   * This is the inverse of createMinecraftContentOp - instead of generating
   * native content from a schema, it analyzes native content to produce a schema.
   */
  async _getEffectiveContentSchemaOp(args) {
    if (!this._creatorTools) {
      throw new Error("Creator Tools is not initialized");
    }
    try {
      import_Log.default.message(
        `[MCP] Tool getEffectiveContentSchema called with args: ${JSON.stringify(args)}`
      );
      if (!fs.existsSync(args.folderPath)) {
        return {
          content: [{ type: "text", text: `Error: Folder not found at path: ${args.folderPath}` }],
          isError: true
        };
      }
      const projectOrError = await this._creatorTools.createProjectFromPath(args.folderPath);
      if (!projectOrError || typeof projectOrError === "string") {
        return {
          content: [
            {
              type: "text",
              text: `Error: Failed to load project from path: ${args.folderPath}` + (typeof projectOrError === "string" ? `. ${projectOrError}` : "")
            }
          ],
          isError: true
        };
      }
      const options = {
        minTraitConfidence: args.options?.minTraitConfidence ?? 0.6,
        includeRawComponents: args.options?.includeRawComponents ?? true,
        inferNamespace: args.options?.inferNamespace ?? true,
        includeBehaviorPresets: args.options?.includeBehaviorPresets ?? true,
        includeComponentGroups: args.options?.includeComponentGroups ?? false,
        includeEvents: args.options?.includeEvents ?? false
      };
      const result = await import_ContentSchemaInferrer.default.inferFromProject(projectOrError, options);
      let summaryText = `Analyzed content in "${args.folderPath}":
`;
      summaryText += `- ${result.metadata.entitiesAnalyzed} entities analyzed
`;
      summaryText += `- ${result.metadata.blocksAnalyzed} blocks analyzed
`;
      summaryText += `- ${result.metadata.itemsAnalyzed} items analyzed
`;
      summaryText += `- Inference took ${result.metadata.inferenceTimeMs}ms
`;
      const entityTraits = Object.entries(result.metadata.allDetectedTraits.entity);
      if (entityTraits.length > 0) {
        summaryText += `
Entity traits detected:
`;
        for (const [trait, count] of entityTraits.sort((a, b) => b[1] - a[1])) {
          summaryText += `  - ${trait}: ${count} entities
`;
        }
      }
      const blockTraits = Object.entries(result.metadata.allDetectedTraits.block);
      if (blockTraits.length > 0) {
        summaryText += `
Block traits detected:
`;
        for (const [trait, count] of blockTraits.sort((a, b) => b[1] - a[1])) {
          summaryText += `  - ${trait}: ${count} blocks
`;
        }
      }
      const itemTraits = Object.entries(result.metadata.allDetectedTraits.item);
      if (itemTraits.length > 0) {
        summaryText += `
Item traits detected:
`;
        for (const [trait, count] of itemTraits.sort((a, b) => b[1] - a[1])) {
          summaryText += `  - ${trait}: ${count} items
`;
        }
      }
      if (result.metadata.warnings.length > 0) {
        summaryText += `
Warnings:
${result.metadata.warnings.map((w) => `- ${w}`).join("\n")}`;
      }
      return {
        content: [
          { type: "text", text: summaryText },
          {
            type: "text",
            text: "\n\nInferred Content Schema:\n```json\n" + JSON.stringify(result.definition, null, 2) + "\n```"
          }
        ],
        structuredContent: {
          definition: result.definition,
          metadata: result.metadata
        }
      };
    } catch (error) {
      return {
        content: [{ type: "text", text: `Error analyzing content: ${error}` }],
        isError: true
      };
    }
  }
  /**
   * Reads an image file and returns its contents as base64-encoded image data
   * that can be displayed by the AI.
   *
   * Requires allowImageFileReadsInDescendentFolders to be set to true in
   * .mct/mcp/prefs.json in the file's parent folder or up to 3 levels above.
   */
  async _readImageFileOp(args) {
    const filePath = args.filePath;
    if (!this.isMcpPrefEnabled(filePath, "allowImageFileReadsInDescendentFolders")) {
      return {
        content: [
          {
            type: "text",
            text: `Error: Image file reading is not authorized for this path.

To enable image file reading, create a file at .mct/mcp/prefs.json in the project folder (or up to 3 parent folders above the image) with the following content:

{
  "allowImageFileReadsInDescendentFolders": true
}`
          }
        ],
        isError: true
      };
    }
    if (!fs.existsSync(filePath)) {
      return {
        content: [{ type: "text", text: `Error: File not found at path: ${filePath}` }],
        isError: true
      };
    }
    const stats = fs.statSync(filePath);
    if (stats.isDirectory()) {
      return {
        content: [{ type: "text", text: `Error: Path is a directory, not a file: ${filePath}` }],
        isError: true
      };
    }
    const mimeType = import_StorageUtilities.default.getMimeTypeFromName(filePath);
    if (!import_StorageUtilities.default.isImageMimeType(mimeType)) {
      return {
        content: [
          {
            type: "text",
            text: `Error: Not an image file: ${filePath}
MIME type: ${mimeType}

This tool only supports image files (.png, .jpg, .gif, .webp, .bmp, .tiff, etc.).`
          }
        ],
        isError: true
      };
    }
    try {
      const fileBuffer = fs.readFileSync(filePath);
      const base64Data = fileBuffer.toString("base64");
      const fitted = MinecraftMcpServer.ensureImageFitsContext(base64Data, mimeType);
      const content = [];
      const fileName = path.basename(filePath);
      const fileSizeBytes = fileBuffer.length;
      let dimensionsStr = "";
      try {
        if (mimeType === "image/png" && fileSizeBytes >= 24) {
          const width = fileBuffer.readUInt32BE(16);
          const height = fileBuffer.readUInt32BE(20);
          dimensionsStr = `${width}x${height}`;
        } else if ((mimeType === "image/jpeg" || mimeType === "image/jpg") && fileSizeBytes > 2) {
          dimensionsStr = MinecraftMcpServer._getJpegDimensions(fileBuffer);
        } else if (mimeType === "image/gif" && fileSizeBytes >= 10) {
          const width = fileBuffer.readUInt16LE(6);
          const height = fileBuffer.readUInt16LE(8);
          dimensionsStr = `${width}x${height}`;
        } else if (mimeType === "image/bmp" && fileSizeBytes >= 26) {
          const width = fileBuffer.readUInt32LE(18);
          const height = Math.abs(fileBuffer.readInt32LE(22));
          dimensionsStr = `${width}x${height}`;
        }
      } catch {
      }
      let sizeStr;
      if (fileSizeBytes >= 1024 * 1024) {
        sizeStr = `${(fileSizeBytes / (1024 * 1024)).toFixed(2)}MB`;
      } else if (fileSizeBytes >= 1024) {
        sizeStr = `${(fileSizeBytes / 1024).toFixed(1)}KB`;
      } else {
        sizeStr = `${fileSizeBytes}B`;
      }
      const modifiedDate = stats.mtime.toLocaleDateString("en-US", {
        year: "numeric",
        month: "short",
        day: "numeric"
      });
      const modifiedTime = stats.mtime.toLocaleTimeString("en-US", {
        hour: "2-digit",
        minute: "2-digit",
        hour12: false
      });
      let metaLines = `Image: ${fileName}`;
      if (dimensionsStr) {
        metaLines += ` | ${dimensionsStr}`;
      }
      metaLines += ` @ ${sizeStr} | ${modifiedDate} ${modifiedTime} | ${mimeType}`;
      if (fitted.wasDownscaled) {
        metaLines += `
Note: The original image was automatically downscaled to fit within AI context limits. The original file on disk is unchanged.`;
      }
      content.push({
        type: "text",
        text: metaLines
      });
      content.push({
        type: "image",
        data: fitted.base64,
        mimeType: fitted.mimeType
      });
      return { content };
    } catch (error) {
      return {
        content: [{ type: "text", text: `Error reading image file: ${error}` }],
        isError: true
      };
    }
  }
  /**
   * Extract dimensions from a JPEG buffer by scanning for SOF markers.
   * Returns a "widthxheight" string, or empty string if extraction fails.
   */
  static _getJpegDimensions(buffer) {
    try {
      let offset = 2;
      while (offset < buffer.length - 1) {
        if (buffer[offset] !== 255) break;
        const marker = buffer[offset + 1];
        if (marker >= 192 && marker <= 195) {
          if (offset + 9 <= buffer.length) {
            const height = buffer.readUInt16BE(offset + 5);
            const width = buffer.readUInt16BE(offset + 7);
            return `${width}x${height}`;
          }
          break;
        }
        if (offset + 3 >= buffer.length) break;
        const segmentLength = buffer.readUInt16BE(offset + 2);
        offset += 2 + segmentLength;
      }
    } catch {
    }
    return "";
  }
  /**
   * Maximum base64 size for images returned from MCP tools to stay safely under
   * API request limits (e.g., Claude's ~4MB total request limit).
   * Base64 encoding inflates binary data by ~33%, and we need headroom for the rest
   * of the request payload (tool definitions, conversation history, etc.), so we
   * cap individual images at ~1.5MB base64. This is more conservative than the 3MB
   * limit to account for conversations with multiple images.
   */
  static MAX_IMAGE_BASE64_BYTES = 1.5 * 1024 * 1024;
  /**
   * Ensure a base64-encoded image fits within AI context limits.
   * If the image is too large, it will be downscaled and re-encoded as PNG.
   *
   * This should be called before returning any image from an MCP tool to prevent
   * 413 Request Entity Too Large errors from the AI backend.
   *
   * @param base64Data Base64-encoded image data
   * @param mimeType MIME type of the image
   * @returns Object with (possibly downscaled) base64 data and mimeType. The mimeType
   *          may change to "image/png" if the image was re-encoded.
   */
  static ensureImageFitsContext(base64Data, mimeType) {
    if (base64Data.length <= MinecraftMcpServer.MAX_IMAGE_BASE64_BYTES) {
      return { base64: base64Data, mimeType, wasDownscaled: false };
    }
    import_Log.default.debug(
      `Image too large for AI context (${(base64Data.length / (1024 * 1024)).toFixed(1)}MB base64). Downscaling to fit under ${(MinecraftMcpServer.MAX_IMAGE_BASE64_BYTES / (1024 * 1024)).toFixed(1)}MB.`
    );
    try {
      const imageBuffer = Buffer.from(base64Data, "base64");
      const result = MinecraftMcpServer._downscaleImageToFit(
        imageBuffer,
        mimeType,
        MinecraftMcpServer.MAX_IMAGE_BASE64_BYTES
      );
      if (result) {
        return { base64: result.base64, mimeType: "image/png", wasDownscaled: true };
      }
    } catch (e) {
      import_Log.default.debug(`ensureImageFitsContext failed: ${e}`);
    }
    return { base64: base64Data, mimeType, wasDownscaled: false };
  }
  /**
   * Downscale an image buffer so its base64 representation fits within maxBase64Bytes.
   * Decodes the image to RGBA pixels, computes a scale factor, resizes using bilinear
   * interpolation, and re-encodes as PNG.
   *
   * @param imageBuffer Raw image file bytes (PNG, JPEG, etc.)
   * @param mimeType MIME type of the source image
   * @param maxBase64Bytes Maximum allowed base64 string length
   * @returns Object with base64 string, or undefined if downscaling fails
   */
  static _downscaleImageToFit(imageBuffer, mimeType, maxBase64Bytes) {
    try {
      let srcWidth;
      let srcHeight;
      let srcPixels;
      if (mimeType === "image/png") {
        const png = import_pngjs.PNG.sync.read(imageBuffer);
        srcWidth = png.width;
        srcHeight = png.height;
        srcPixels = new Uint8Array(png.data);
      } else {
        try {
          const png = import_pngjs.PNG.sync.read(imageBuffer);
          srcWidth = png.width;
          srcHeight = png.height;
          srcPixels = new Uint8Array(png.data);
        } catch {
          import_Log.default.debug(`Cannot decode ${mimeType} for downscaling \u2014 pngjs only supports PNG directly.`);
          return void 0;
        }
      }
      if (srcWidth <= 0 || srcHeight <= 0) {
        return void 0;
      }
      const maxBinaryBytes = Math.floor(maxBase64Bytes * 3 / 4);
      const estimatedBytesPerPixel = 3;
      const maxPixels = Math.floor(maxBinaryBytes / estimatedBytesPerPixel);
      const currentPixels = srcWidth * srcHeight;
      let scale = Math.sqrt(maxPixels / currentPixels);
      scale = Math.min(scale, 1);
      scale *= 0.9;
      let dstWidth = Math.max(1, Math.floor(srcWidth * scale));
      let dstHeight = Math.max(1, Math.floor(srcHeight * scale));
      for (let attempt = 0; attempt < 5; attempt++) {
        const resizedPixels = MinecraftMcpServer._bilinearResize(srcPixels, srcWidth, srcHeight, dstWidth, dstHeight);
        const outPng = new import_pngjs.PNG({ width: dstWidth, height: dstHeight });
        outPng.data = Buffer.from(resizedPixels);
        const pngBuffer = import_pngjs.PNG.sync.write(outPng);
        const base64 = pngBuffer.toString("base64");
        if (base64.length <= maxBase64Bytes) {
          import_Log.default.debug(
            `Downscaled image from ${srcWidth}x${srcHeight} to ${dstWidth}x${dstHeight} (${(base64.length / (1024 * 1024)).toFixed(2)}MB base64)`
          );
          return { base64 };
        }
        const overshoot = base64.length / maxBase64Bytes;
        const shrinkFactor = Math.sqrt(1 / overshoot) * 0.9;
        dstWidth = Math.max(1, Math.floor(dstWidth * shrinkFactor));
        dstHeight = Math.max(1, Math.floor(dstHeight * shrinkFactor));
      }
      import_Log.default.debug("Failed to downscale image to fit within limits after 5 attempts");
      return void 0;
    } catch (e) {
      import_Log.default.debug(`Image downscaling failed: ${e}`);
      return void 0;
    }
  }
  /**
   * Resize RGBA pixel data using bilinear interpolation.
   * Produces smoother results than nearest-neighbor for downscaling screenshots and textures.
   *
   * @param srcPixels Source RGBA pixel data (4 bytes per pixel)
   * @param srcW Source width
   * @param srcH Source height
   * @param dstW Destination width
   * @param dstH Destination height
   * @returns Resized RGBA pixel data
   */
  static _bilinearResize(srcPixels, srcW, srcH, dstW, dstH) {
    const dst = new Uint8Array(dstW * dstH * 4);
    const xRatio = srcW / dstW;
    const yRatio = srcH / dstH;
    for (let dstY = 0; dstY < dstH; dstY++) {
      const srcYf = (dstY + 0.5) * yRatio - 0.5;
      const srcY0 = Math.max(0, Math.floor(srcYf));
      const srcY1 = Math.min(srcH - 1, srcY0 + 1);
      const yLerp = srcYf - srcY0;
      for (let dstX = 0; dstX < dstW; dstX++) {
        const srcXf = (dstX + 0.5) * xRatio - 0.5;
        const srcX0 = Math.max(0, Math.floor(srcXf));
        const srcX1 = Math.min(srcW - 1, srcX0 + 1);
        const xLerp = srcXf - srcX0;
        const idx00 = (srcY0 * srcW + srcX0) * 4;
        const idx10 = (srcY0 * srcW + srcX1) * 4;
        const idx01 = (srcY1 * srcW + srcX0) * 4;
        const idx11 = (srcY1 * srcW + srcX1) * 4;
        const dstIdx = (dstY * dstW + dstX) * 4;
        for (let c = 0; c < 4; c++) {
          const top = srcPixels[idx00 + c] * (1 - xLerp) + srcPixels[idx10 + c] * xLerp;
          const bot = srcPixels[idx01 + c] * (1 - xLerp) + srcPixels[idx11 + c] * xLerp;
          dst[dstIdx + c] = Math.round(top * (1 - yLerp) + bot * yLerp);
        }
      }
    }
    return dst;
  }
  /**
   * Writes base64-encoded image data to a file.
   *
   * Requires allowImageFileWritesInDescendentFolders to be set to true in
   * .mct/mcp/prefs.json in the file's parent folder or up to 3 levels above.
   */
  async _writeImageFileFromBase64Op(args) {
    const filePath = args.filePath;
    if (!this.isMcpPrefEnabled(filePath, "allowImageFileWritesInDescendentFolders")) {
      return {
        content: [
          {
            type: "text",
            text: `Error: Image file writing is not authorized for this path.

To enable image file writing, create a file at .mct/mcp/prefs.json in the project folder (or up to 3 parent folders above the target location) with the following content:

{
  "allowImageFileWritesInDescendentFolders": true
}`
          }
        ],
        isError: true
      };
    }
    const mimeType = args.mimeType || import_StorageUtilities.default.getMimeTypeFromName(filePath);
    if (!import_StorageUtilities.default.isImageMimeType(mimeType)) {
      return {
        content: [
          {
            type: "text",
            text: `Error: Not a valid image file extension: ${filePath}
MIME type: ${mimeType}

This tool only supports image files (.png, .jpg, .webp, .bmp, .tiff, etc.).`
          }
        ],
        isError: true
      };
    }
    try {
      const parentDir = path.dirname(filePath);
      if (!fs.existsSync(parentDir)) {
        fs.mkdirSync(parentDir, { recursive: true });
      }
      const buffer = Buffer.from(args.base64Data, "base64");
      fs.writeFileSync(filePath, buffer);
      const stats = fs.statSync(filePath);
      const resultBase64 = buffer.toString("base64");
      const fitted = MinecraftMcpServer.ensureImageFitsContext(resultBase64, mimeType);
      return {
        content: [
          {
            type: "text",
            text: `Successfully wrote image file:
  Path: ${filePath}
  Size: ${stats.size} bytes
  MIME type: ${mimeType}`
          },
          {
            type: "image",
            data: fitted.base64,
            mimeType: fitted.mimeType
          }
        ]
      };
    } catch (error) {
      return {
        content: [{ type: "text", text: `Error writing image file: ${error}` }],
        isError: true
      };
    }
  }
  /**
   * Converts SVG markup to PNG and writes it to a file.
   *
   * Requires allowImageFileWritesInDescendentFolders to be set to true in
   * .mct/mcp/prefs.json in the file's parent folder or up to 3 levels above.
   */
  async _writeImageFileFromSvgOp(args) {
    const filePath = args.filePath;
    if (!this.isMcpPrefEnabled(filePath, "allowImageFileWritesInDescendentFolders")) {
      return {
        content: [
          {
            type: "text",
            text: `Error: Image file writing is not authorized for this path.

To enable image file writing, create a file at .mct/mcp/prefs.json in the project folder (or up to 3 parent folders above the target location) with the following content:

{
  "allowImageFileWritesInDescendentFolders": true
}`
          }
        ],
        isError: true
      };
    }
    try {
      const parentDir = path.dirname(filePath);
      if (!fs.existsSync(parentDir)) {
        fs.mkdirSync(parentDir, { recursive: true });
      }
      const opts = {};
      if (args.width || args.height) {
        opts.fitTo = args.width && args.height ? { mode: "width", value: args.width } : args.width ? { mode: "width", value: args.width } : { mode: "height", value: args.height };
      }
      const { Resvg } = await import("@resvg/resvg-js");
      const resvg = new Resvg(args.svgContent, opts);
      const pngData = resvg.render();
      const buffer = Buffer.from(pngData.asPng());
      fs.writeFileSync(filePath, buffer);
      const stats = fs.statSync(filePath);
      const resultBase64 = buffer.toString("base64");
      const fitted = MinecraftMcpServer.ensureImageFitsContext(resultBase64, "image/png");
      return {
        content: [
          {
            type: "text",
            text: `Successfully wrote image file:
  Path: ${filePath}
  Size: ${stats.size} bytes
  Dimensions: ${pngData.width}x${pngData.height}
  (converted from SVG to PNG)`
          },
          {
            type: "image",
            data: fitted.base64,
            mimeType: fitted.mimeType
          }
        ]
      };
    } catch (error) {
      return {
        content: [{ type: "text", text: `Error writing image file: ${error}` }],
        isError: true
      };
    }
  }
  /**
   * Writes an image file from a pixel art definition with paletted pixels.
   *
   * The pixel art format uses ASCII-style lines where each character maps to a color
   * in a palette. Spaces are transparent. This is ideal for creating Minecraft-style
   * pixel art textures.
   *
   * Requires allowImageFileWritesInDescendentFolders to be set to true in
   * .mct/mcp/prefs.json in the file's parent folder or up to 3 levels above.
   */
  async _writeImageFileFromPixelArtOp(args) {
    const filePath = args.filePath;
    if (!this.isMcpPrefEnabled(filePath, "allowImageFileWritesInDescendentFolders")) {
      return {
        content: [
          {
            type: "text",
            text: `Error: Image file writing is not authorized for this path.

To enable image file writing, create a file at .mct/mcp/prefs.json in the project folder (or up to 3 parent folders above the target location) with the following content:

{
  "allowImageFileWritesInDescendentFolders": true
}`
          }
        ],
        isError: true
      };
    }
    const ext = path.extname(filePath).toLowerCase();
    if (ext !== ".png") {
      return {
        content: [
          {
            type: "text",
            text: `Error: Pixel art output must be a PNG file. Got extension: ${ext}`
          }
        ],
        isError: true
      };
    }
    if (!args.lines || args.lines.length === 0) {
      return {
        content: [{ type: "text", text: "Error: lines array is required and must not be empty" }],
        isError: true
      };
    }
    if (!args.palette || Object.keys(args.palette).length === 0) {
      return {
        content: [{ type: "text", text: "Error: palette is required and must not be empty" }],
        isError: true
      };
    }
    try {
      const scale = args.scale || 1;
      const artHeight = args.lines.length;
      let artWidth = 0;
      for (const line of args.lines) {
        artWidth = Math.max(artWidth, line.length);
      }
      if (artWidth === 0) {
        return {
          content: [{ type: "text", text: "Error: All lines are empty" }],
          isError: true
        };
      }
      const width = artWidth * scale;
      const height = artHeight * scale;
      const pixels = new Uint8Array(width * height * 4);
      const parseColor = (color) => {
        if (!color) return { r: 0, g: 0, b: 0, a: 0 };
        if (color.hex) {
          let hex = color.hex;
          if (hex.startsWith("#")) hex = hex.slice(1);
          if (hex.length === 3) {
            hex = hex[0] + hex[0] + hex[1] + hex[1] + hex[2] + hex[2];
          }
          const r = parseInt(hex.slice(0, 2), 16) || 0;
          const g = parseInt(hex.slice(2, 4), 16) || 0;
          const b = parseInt(hex.slice(4, 6), 16) || 0;
          const a = hex.length >= 8 ? parseInt(hex.slice(6, 8), 16) : 255;
          return { r, g, b, a };
        }
        return {
          r: color.r ?? 0,
          g: color.g ?? 0,
          b: color.b ?? 0,
          a: color.a ?? 255
        };
      };
      const parsedPalette = {};
      for (const char in args.palette) {
        parsedPalette[char] = parseColor(args.palette[char]);
      }
      const bgColor = args.backgroundColor ? parseColor(args.backgroundColor) : { r: 0, g: 0, b: 0, a: 0 };
      for (let i = 0; i < width * height; i++) {
        pixels[i * 4] = bgColor.r;
        pixels[i * 4 + 1] = bgColor.g;
        pixels[i * 4 + 2] = bgColor.b;
        pixels[i * 4 + 3] = bgColor.a;
      }
      for (let lineIdx = 0; lineIdx < args.lines.length; lineIdx++) {
        const line = args.lines[lineIdx];
        for (let charIdx = 0; charIdx < line.length; charIdx++) {
          const char = line[charIdx];
          if (char === " ") continue;
          const color = parsedPalette[char];
          if (!color) continue;
          const startX = charIdx * scale;
          const startY = lineIdx * scale;
          const endX = startX + scale;
          const endY = startY + scale;
          for (let y = startY; y < endY; y++) {
            for (let x = startX; x < endX; x++) {
              const idx = (y * width + x) * 4;
              pixels[idx] = color.r;
              pixels[idx + 1] = color.g;
              pixels[idx + 2] = color.b;
              pixels[idx + 3] = color.a;
            }
          }
        }
      }
      const pngData = import_ImageGenerationUtilities.default.encodeRgbaToPng(pixels, width, height);
      if (!pngData) {
        return {
          content: [{ type: "text", text: "Error: Failed to encode PNG" }],
          isError: true
        };
      }
      const parentDir = path.dirname(filePath);
      if (!fs.existsSync(parentDir)) {
        fs.mkdirSync(parentDir, { recursive: true });
      }
      fs.writeFileSync(filePath, Buffer.from(pngData));
      const stats = fs.statSync(filePath);
      const resultBase64 = Buffer.from(pngData).toString("base64");
      const fitted = MinecraftMcpServer.ensureImageFitsContext(resultBase64, "image/png");
      return {
        content: [
          {
            type: "text",
            text: `Successfully wrote pixel art image:
  Path: ${filePath}
  Size: ${stats.size} bytes
  Dimensions: ${width}x${height} (${artWidth}x${artHeight} at ${scale}x scale)
  Palette: ${Object.keys(args.palette).length} colors`
          },
          {
            type: "image",
            data: fitted.base64,
            mimeType: fitted.mimeType
          }
        ]
      };
    } catch (error) {
      return {
        content: [{ type: "text", text: `Error writing pixel art image: ${error}` }],
        isError: true
      };
    }
  }
  /**
   * Preview a model design by converting it to geometry and rendering a preview image.
   * Returns the preview as a base64-encoded PNG image.
   */
  async _previewModelDesignOp(args) {
    if (!this._creatorTools || !this._env) {
      return {
        content: [{ type: "text", text: "Error: Creator Tools is not initialized" }],
        isError: true
      };
    }
    const design = args.design;
    if (!design) {
      return {
        content: [{ type: "text", text: "Error: design is required" }],
        isError: true
      };
    }
    const validationErrors = import_ModelDesignUtilities.default.validateDesign(design);
    if (validationErrors.length > 0) {
      return {
        content: [
          {
            type: "text",
            text: `Design validation errors:
${validationErrors.map((e) => `  - ${e}`).join("\n")}`
          }
        ],
        isError: true
      };
    }
    const conversionResult = import_ModelDesignUtilities.default.convertToGeometry(design);
    const geometryJson = JSON.stringify(conversionResult.geometry, null, 2);
    const _atlasSvg = import_ModelDesignUtilities.default.generateAtlasSvg(
      conversionResult.atlasRegions,
      conversionResult.textureSize
    );
    const textureDataUrl = await import_ImageGenerationUtilities.default.generateTextureFromAtlas(
      conversionResult.atlasRegions,
      conversionResult.textureSize,
      conversionResult.pixelsPerUnit
    );
    const serverManager = this.ensureServerManager();
    serverManager.features = import_ServerManager2.ServerManagerFeatures.all;
    const httpServer = serverManager.ensureHttpServer(this._previewServerPort);
    const renderToken = Math.random().toString(36).substring(2, 15) + Date.now().toString(36);
    const geometryPath = `/temp/preview-geometry-${renderToken}.json`;
    const texturePath = `/temp/preview-texture-${renderToken}.png`;
    httpServer.registerTempContent(geometryPath, geometryJson, "application/json");
    if (textureDataUrl) {
      const base64Match = textureDataUrl.match(/^data:image\/png;base64,(.*)$/);
      if (base64Match) {
        const binaryData = Buffer.from(base64Match[1], "base64");
        httpServer.registerTempContent(texturePath, binaryData, "image/png");
      }
    }
    const baseUrl = `http://localhost:${httpServer.port}`;
    if (!httpServer.isListening) {
      await httpServer.waitForReady();
    }
    if (!httpServer.isListening) {
      import_Log.default.debugAlert(`HTTP server on port ${httpServer.port} is not actually listening. Server may have crashed.`);
      return {
        content: [
          {
            type: "text",
            text: `Error: Internal HTTP server on port ${httpServer.port} is not responding. Please restart the MCP server.`
          }
        ],
        isError: true
      };
    }
    if (!this._cachedRenderer) {
      this._cachedRenderer = new import_PlaywrightPageRenderer.default(baseUrl);
      const initialized = await this._cachedRenderer.initialize();
      if (!initialized) {
        httpServer.clearTempContent();
        this._cachedRenderer = void 0;
        return {
          content: [
            {
              type: "text",
              text: "Error: Could not initialize headless renderer. Ensure Chrome/Edge is available."
            }
          ],
          isError: true
        };
      }
      await this._cachedRenderer.warmUp();
    }
    if (!this._cachedRenderer.isBrowserReady()) {
      console.warn("Browser disconnected, reinitializing...");
      this._cachedRenderer = new import_PlaywrightPageRenderer.default(baseUrl);
      const reinitialized = await this._cachedRenderer.initialize();
      if (!reinitialized) {
        httpServer.clearTempContent();
        this._cachedRenderer = void 0;
        return {
          content: [{ type: "text", text: "Error: Browser disconnected and could not be reinitialized." }],
          isError: true
        };
      }
      await this._cachedRenderer.warmUp();
    }
    const renderer = this._cachedRenderer;
    let modelViewerUrl = `/?mode=modelviewer&geometry=${encodeURIComponent(geometryPath)}&skipVanilla=true`;
    if (textureDataUrl) {
      modelViewerUrl += `&texture=${encodeURIComponent(texturePath)}`;
    }
    const width = args.width || 768;
    const height = args.height || 768;
    const multiAngle = args.multiAngle === true;
    const imageFormat = args.imageFormat || "png";
    const jpegQuality = args.jpegQuality || 80;
    const modelBoundsPixels = import_ModelDesignUtilities.default.calculateModelBounds(design);
    const modelBounds = {
      minX: modelBoundsPixels.minX / 16,
      minY: modelBoundsPixels.minY / 16,
      minZ: modelBoundsPixels.minZ / 16,
      maxX: modelBoundsPixels.maxX / 16,
      maxY: modelBoundsPixels.maxY / 16,
      maxZ: modelBoundsPixels.maxZ / 16,
      maxDimension: modelBoundsPixels.maxDimension / 16,
      center: {
        x: modelBoundsPixels.center.x / 16,
        y: modelBoundsPixels.center.y / 16,
        z: modelBoundsPixels.center.z / 16
      }
    };
    const modelSize = modelBounds.maxDimension;
    const radius = Math.max(modelSize * 1.2 + 1.5, 2.5);
    const entityWorldX = 3.5;
    const entityWorldY = 2;
    const entityWorldZ = 3.5;
    const centerX = entityWorldX + modelBounds.center.x;
    const centerY = entityWorldY + modelBounds.center.y;
    const centerZ = entityWorldZ + modelBounds.center.z;
    const minCameraY = 0.5;
    const isometricY = Math.max(centerY + radius * 0.8, minCameraY);
    const sideViewY = Math.max(centerY + radius * 0.3, minCameraY);
    const anglePresets = {
      // Front views - true isometric angle (~35 degrees from horizontal)
      "front-right": {
        x: centerX + radius * 0.7,
        y: isometricY,
        z: centerZ + radius * 0.7,
        label: "Front Right"
      },
      "front-left": {
        x: centerX - radius * 0.7,
        y: isometricY,
        z: centerZ + radius * 0.7,
        label: "Front Left"
      },
      // Back views - isometric
      "back-right": {
        x: centerX + radius * 0.7,
        y: isometricY,
        z: centerZ - radius * 0.7,
        label: "Back Right"
      },
      "back-left": {
        x: centerX - radius * 0.7,
        y: isometricY,
        z: centerZ - radius * 0.7,
        label: "Back Left"
      },
      // Top-down view - steeper angle
      "top-down": {
        x: centerX + radius * 0.4,
        y: centerY + radius * 0.85,
        z: centerZ + radius * 0.4,
        label: "Top Down"
      },
      // Pure side views - below center but clamped
      "side-right": { x: centerX + radius, y: sideViewY, z: centerZ, label: "Side Right" },
      "side-left": { x: centerX - radius, y: sideViewY, z: centerZ, label: "Side Left" },
      // Front and back center views
      front: { x: centerX, y: isometricY, z: centerZ + radius, label: "Front" },
      back: { x: centerX, y: isometricY, z: centerZ - radius, label: "Back" }
    };
    let result;
    let actualWidth = width;
    let actualHeight = height;
    if (multiAngle) {
      const requestedAngles = args.anglePresets || ["front-right", "front-left", "back-right", "back-left"];
      const anglesToRender = requestedAngles.filter((a) => anglePresets[a.toLowerCase()]).map((a) => ({ key: a.toLowerCase(), ...anglePresets[a.toLowerCase()] }));
      if (anglesToRender.length === 0) {
        anglesToRender.push({ key: "front-right", ...anglePresets["front-right"] });
        anglesToRender.push({ key: "back-left", ...anglePresets["back-left"] });
        anglesToRender.push({ key: "side-right", ...anglePresets["side-right"] });
        anglesToRender.push({ key: "top-down", ...anglePresets["top-down"] });
      }
      const usePyramidLayout = anglesToRender.length === 3;
      const useGrid = anglesToRender.length === 4;
      const cols = useGrid ? 2 : usePyramidLayout ? 2 : anglesToRender.length;
      const rows = useGrid ? 2 : usePyramidLayout ? 2 : 1;
      const renderedImages = [];
      const boneCount = design.bones?.length || 1;
      const cubeCount = design.bones?.reduce((sum, b) => sum + (b.cubes?.length || 0), 0) || 1;
      const baseCanvasTimeout = 8e3;
      const additionalCanvasTimeout = Math.min(boneCount * 500 + cubeCount * 100, 12e3);
      const canvasTimeout = Math.round(baseCanvasTimeout + additionalCanvasTimeout);
      const baseWaitTime = 1500;
      const additionalWaitTime = Math.min(boneCount * 200 + cubeCount * 50, 4e3);
      const renderWaitTime = Math.round(baseWaitTime + additionalWaitTime);
      const renderErrors = [];
      for (let i = 0; i < anglesToRender.length; i++) {
        const angle = anglesToRender[i];
        const isWideImage = usePyramidLayout && i === 2;
        const imgWidth = isWideImage ? width : Math.floor(width / cols);
        const imgHeight = Math.floor(height / rows);
        const angleUrl = modelViewerUrl + `&cameraX=${angle.x}&cameraY=${angle.y}&cameraZ=${angle.z}`;
        if (i > 0) {
          await new Promise((resolve) => setTimeout(resolve, 300));
        }
        const angleResult = await renderer.renderModelFast(angleUrl, {
          width: imgWidth,
          height: imgHeight,
          renderWaitTime,
          canvasTimeout,
          imageFormat: "png",
          // Use PNG for intermediate renders, compress after stitching
          forceNewContext: true
          // Force fresh context for each angle to ensure clean render state
        });
        if (angleResult.imageData) {
          renderedImages.push({ label: angle.label, imageData: angleResult.imageData, isWide: isWideImage });
        } else {
          const errorMsg = angleResult.error || "unknown error";
          renderErrors.push(`${angle.label}: ${errorMsg}`);
          console.warn(
            `Multi-angle render failed for angle ${angle.key} (${i + 1}/${anglesToRender.length}): ${errorMsg}`
          );
          if (i === 0) {
            console.warn(`  URL: ${angleUrl.substring(0, 100)}...`);
            console.warn(`  Dimensions: ${imgWidth}x${imgHeight}`);
            console.warn(`  Timeout: ${canvasTimeout}ms, RenderWait: ${renderWaitTime}ms`);
          }
        }
      }
      if (renderedImages.length > 0) {
        const singleWidth = Math.floor(width / cols);
        const singleHeight = Math.floor(height / rows);
        const stitchedImage = await import_ImageGenerationUtilities.default.stitchImagesWithLabels(
          renderedImages,
          singleWidth,
          singleHeight,
          cols,
          rows,
          usePyramidLayout ? "pyramid" : void 0
        );
        result = { imageData: stitchedImage };
        const labelHeight = 30;
        actualWidth = singleWidth * cols;
        actualHeight = (singleHeight + labelHeight) * rows;
      } else {
        let errorMsg = `No images rendered for multi-angle view. `;
        errorMsg += `Attempted ${anglesToRender.length} angles. `;
        if (renderErrors.length > 0) {
          errorMsg += `Errors: ${renderErrors.join("; ")}`;
        } else {
          errorMsg += `No specific errors captured - browser may not be fully initialized.`;
        }
        result = { imageData: void 0, error: errorMsg };
      }
    } else {
      const defaultAngle = anglePresets["front-right"];
      const singleAngleUrl = modelViewerUrl + `&cameraX=${defaultAngle.x}&cameraY=${defaultAngle.y}&cameraZ=${defaultAngle.z}`;
      const boneCount = design.bones?.length || 1;
      const cubeCount = design.bones?.reduce((sum, b) => sum + (b.cubes?.length || 0), 0) || 1;
      const baseCanvasTimeout = 8e3;
      const additionalCanvasTimeout = Math.min(boneCount * 500 + cubeCount * 100, 12e3);
      const canvasTimeout = Math.round(baseCanvasTimeout + additionalCanvasTimeout);
      const baseWaitTime = 1500;
      const additionalWaitTime = Math.min(boneCount * 200 + cubeCount * 50, 4e3);
      const renderWaitTime = Math.round(baseWaitTime + additionalWaitTime);
      result = await renderer.renderModelFast(singleAngleUrl, {
        width,
        height,
        renderWaitTime,
        canvasTimeout,
        imageFormat,
        jpegQuality,
        forceNewContext: true
        // Always use fresh context since temp content changes between calls
      });
    }
    httpServer.clearTempContent();
    if (result.error) {
      return {
        content: [
          {
            type: "text",
            text: `Rendering failed: ${result.error}

Generated geometry:
${geometryJson}`
          }
        ],
        isError: true
      };
    }
    if (result.imageData) {
      let finalImageData = result.imageData;
      const swatches = import_ImageGenerationUtilities.default.extractTextureSwatches(design);
      if (swatches.length > 0) {
        finalImageData = await import_ImageGenerationUtilities.default.generateAndAppendSwatchStrip(
          result.imageData,
          swatches,
          actualWidth,
          actualHeight,
          6
        );
      }
      let outputFormat = imageFormat;
      if (imageFormat === "jpeg") {
        if (multiAngle) {
          finalImageData = await import_ImageGenerationUtilities.default.convertPngToJpeg(finalImageData, jpegQuality);
        }
      }
      const rawBase64Image = Buffer.from(finalImageData).toString("base64");
      const outputMimeType = outputFormat === "jpeg" ? "image/jpeg" : "image/png";
      const fitted = MinecraftMcpServer.ensureImageFitsContext(rawBase64Image, outputMimeType);
      let tempImagePath;
      try {
        const tempDir = path.join(process.cwd(), "debugoutput", "mcp-previews");
        if (!fs.existsSync(tempDir)) {
          fs.mkdirSync(tempDir, { recursive: true });
        }
        const fileName = `preview-${Date.now()}-${Math.random().toString(36).substring(2, 8)}.${outputFormat}`;
        tempImagePath = path.join(tempDir, fileName);
        fs.writeFileSync(tempImagePath, finalImageData);
        import_Log.default.debug(`Saved preview image to: ${tempImagePath}`);
      } catch (saveErr) {
        import_Log.default.debug(`Failed to save preview image to temp file: ${saveErr.message}`);
      }
      const responseContent = [
        {
          type: "image",
          data: fitted.base64,
          mimeType: fitted.mimeType
        }
      ];
      if (tempImagePath) {
        responseContent.push({
          type: "text",
          text: `[MCT_IMAGE_PATH:${tempImagePath}]`
        });
      }
      if (conversionResult.warnings.length > 0) {
        responseContent.push({
          type: "text",
          text: `Warnings:
${conversionResult.warnings.map((w) => `  - ${w}`).join("\n")}`
        });
      }
      return {
        content: responseContent,
        structuredContent: {
          geometryJson: conversionResult.geometry,
          textureSize: conversionResult.textureSize,
          warnings: conversionResult.warnings
        }
      };
    }
    return {
      content: [{ type: "text", text: "Error: No image data returned from renderer" }],
      isError: true
    };
  }
  /**
   * Export a model design to .geo.json and texture.png files.
   */
  async _exportModelDesignOp(args) {
    const design = args.design;
    const geometryOutputPath = args.geometryOutputPath;
    const textureOutputPath = args.textureOutputPath;
    const overwrite = args.overwrite;
    if (!design) {
      return {
        content: [{ type: "text", text: "Error: design is required" }],
        isError: true
      };
    }
    if (!geometryOutputPath || !textureOutputPath) {
      return {
        content: [{ type: "text", text: "Error: geometryOutputPath and textureOutputPath are required" }],
        isError: true
      };
    }
    const validationErrors = import_ModelDesignUtilities.default.validateDesign(design);
    if (validationErrors.length > 0) {
      return {
        content: [
          {
            type: "text",
            text: `Design validation errors:
${validationErrors.map((e) => `  - ${e}`).join("\n")}`
          }
        ],
        isError: true
      };
    }
    if (!overwrite) {
      if (fs.existsSync(geometryOutputPath)) {
        return {
          content: [
            {
              type: "text",
              text: `Error: File already exists: ${geometryOutputPath}
Use overwrite: true to replace it.`
            }
          ],
          isError: true
        };
      }
      if (fs.existsSync(textureOutputPath)) {
        return {
          content: [
            {
              type: "text",
              text: `Error: File already exists: ${textureOutputPath}
Use overwrite: true to replace it.`
            }
          ],
          isError: true
        };
      }
    }
    const conversionResult = import_ModelDesignUtilities.default.convertToGeometry(design);
    const geometryJson = JSON.stringify(conversionResult.geometry, null, 2);
    const textureDataUrl = await import_ImageGenerationUtilities.default.generateTextureFromAtlas(
      conversionResult.atlasRegions,
      conversionResult.textureSize,
      conversionResult.pixelsPerUnit
    );
    const filesWritten = [];
    const errors = [];
    try {
      const geoDir = path.dirname(geometryOutputPath);
      if (!fs.existsSync(geoDir)) {
        fs.mkdirSync(geoDir, { recursive: true });
      }
      fs.writeFileSync(geometryOutputPath, geometryJson, "utf-8");
      filesWritten.push(geometryOutputPath);
    } catch (e) {
      errors.push(`Failed to write geometry file: ${e}`);
    }
    if (textureDataUrl) {
      try {
        const texDir = path.dirname(textureOutputPath);
        if (!fs.existsSync(texDir)) {
          fs.mkdirSync(texDir, { recursive: true });
        }
        const base64Match = textureDataUrl.match(/^data:image\/png;base64,(.*)$/);
        if (base64Match) {
          const binaryData = Buffer.from(base64Match[1], "base64");
          fs.writeFileSync(textureOutputPath, binaryData);
          filesWritten.push(textureOutputPath);
        }
      } catch (e) {
        errors.push(`Failed to write texture file: ${e}`);
      }
    } else {
      errors.push("Could not generate texture image");
    }
    if (errors.length > 0 && filesWritten.length === 0) {
      return {
        content: [
          {
            type: "text",
            text: `Export failed:
${errors.map((e) => `  - ${e}`).join("\n")}`
          }
        ],
        isError: true
      };
    }
    let resultText = `Successfully exported model design:
`;
    for (const file of filesWritten) {
      resultText += `  \u2713 ${file}
`;
    }
    if (conversionResult.warnings.length > 0) {
      resultText += `
Warnings:
${conversionResult.warnings.map((w) => `  - ${w}`).join("\n")}`;
    }
    if (errors.length > 0) {
      resultText += `
Errors:
${errors.map((e) => `  - ${e}`).join("\n")}`;
    }
    return {
      content: [{ type: "text", text: resultText }],
      structuredContent: {
        success: filesWritten.length > 0,
        filesWritten,
        geometryJson: conversionResult.geometry,
        warnings: conversionResult.warnings,
        errors
      }
    };
  }
  /**
   * Unified tool: Creates a 3D model, exports files to project, and returns a preview.
   *
   * This combines the functionality of previewModelDesign and exportModelDesign into
   * a single, project-aware operation that:
   * 1. Validates and converts the design to geometry + texture
   * 2. Saves files to the appropriate project folder (auto-detected)
   * 3. Persists the design to an accessory folder for future iteration
   * 4. Auto-wires to matching entity/block/item if found
   * 5. Returns a preview image
   */
  async _designModelOp(args) {
    if (!this._creatorTools) {
      return {
        content: [{ type: "text", text: "Error: Creator Tools is not initialized" }],
        isError: true
      };
    }
    const design = args.design;
    if (!design) {
      return {
        content: [{ type: "text", text: "Error: design is required" }],
        isError: true
      };
    }
    const modelId = args.modelId;
    if (!modelId) {
      return {
        content: [{ type: "text", text: "Error: modelId is required" }],
        isError: true
      };
    }
    const projectPath = args.projectPath;
    if (!projectPath) {
      return {
        content: [{ type: "text", text: "Error: projectPath is required" }],
        isError: true
      };
    }
    const usage = args.usage || "entity";
    const validationErrors = import_ModelDesignUtilities.default.validateDesign(design);
    if (validationErrors.length > 0) {
      return {
        content: [
          {
            type: "text",
            text: `Design validation errors:
${validationErrors.map((e) => `  - ${e}`).join("\n")}`
          }
        ],
        isError: true
      };
    }
    const storage = new import_NodeStorage.default(projectPath, "");
    const projectFolder = storage.rootFolder;
    await projectFolder.ensureExists();
    const project = new import_Project.default(this._creatorTools, path.basename(projectPath), null);
    project.setProjectFolder(projectFolder);
    const rpFolder = await project.ensureDefaultResourcePackFolder();
    await project.inferProjectItemsFromFilesRootFolder();
    const modelsSubPath = usage === "block" ? "/models/blocks/" : usage === "item" ? "/models/item/" : "/models/entity/";
    const texturesSubPath = usage === "block" ? "/textures/blocks/" : usage === "item" ? "/textures/items/" : "/textures/entity/";
    const modelsFolder = await rpFolder.ensureFolderFromRelativePath(modelsSubPath);
    const texturesFolder = await rpFolder.ensureFolderFromRelativePath(texturesSubPath);
    const conversionResult = import_ModelDesignUtilities.default.convertToGeometry(design);
    const geometryJson = JSON.stringify(conversionResult.geometry, null, 2);
    const textureDataUrl = await import_ImageGenerationUtilities.default.generateTextureFromAtlas(
      conversionResult.atlasRegions,
      conversionResult.textureSize,
      conversionResult.pixelsPerUnit
    );
    const filesWritten = [];
    const errors = [];
    const geoFile = modelsFolder.ensureFile(`${modelId}.geo.json`);
    try {
      geoFile.setContent(geometryJson);
      await geoFile.saveContent(false);
      filesWritten.push(geoFile.storageRelativePath || `${modelId}.geo.json`);
    } catch (e) {
      errors.push(`Failed to write geometry file: ${e}`);
    }
    let textureBytes;
    if (textureDataUrl) {
      const texFile = texturesFolder.ensureFile(`${modelId}.png`);
      try {
        const base64Match = textureDataUrl.match(/^data:image\/png;base64,(.*)$/);
        if (base64Match) {
          textureBytes = Buffer.from(base64Match[1], "base64");
          texFile.setContent(textureBytes);
          await texFile.saveContent(false);
          filesWritten.push(texFile.storageRelativePath || `${modelId}.png`);
        }
      } catch (e) {
        errors.push(`Failed to write texture file: ${e}`);
      }
    } else {
      errors.push("Could not generate texture image");
    }
    let wireTarget = void 0;
    if (args.wireTo === false) {
      wireTarget = void 0;
    } else if (typeof args.wireTo === "string") {
      wireTarget = args.wireTo;
    } else {
      const matchingItem = project.items.find((item) => {
        if (usage === "entity" && item.itemType === import_IProjectItemData.ProjectItemType.entityTypeBehavior) {
          return item.name === modelId;
        }
        if (usage === "block" && item.itemType === import_IProjectItemData.ProjectItemType.blockTypeBehavior) {
          return item.name === modelId;
        }
        if (usage === "item" && item.itemType === import_IProjectItemData.ProjectItemType.itemTypeBehavior) {
          return item.name === modelId;
        }
        return false;
      });
      if (matchingItem) {
        wireTarget = matchingItem.name;
        import_Log.default.debug(`designModel: Auto-discovered matching ${usage}: ${wireTarget}`);
      }
    }
    let wiringNote = "";
    if (wireTarget) {
      wiringNote = `

\u{1F4A1} Found matching ${usage} "${wireTarget}" - wiring support coming soon.`;
    }
    await project.inferProjectItemsFromFilesRootFolder();
    const geoProjectItem = project.items.find(
      (item) => item.projectPath && item.projectPath.endsWith(`${modelId}.geo.json`)
    );
    if (geoProjectItem) {
      try {
        const designDef = await import_ModelDesignDefinition.default.ensureAsAccessoryOnProjectItem(geoProjectItem);
        if (designDef) {
          await designDef.updateDesign(design, {
            usage,
            wiredTo: wireTarget
          });
          import_Log.default.debug(`designModel: Saved design to accessory folder for ${modelId}`);
        }
      } catch (e) {
        import_Log.default.debug(`designModel: Failed to save design to accessory folder: ${e}`);
      }
    }
    let previewImageData;
    let previewError;
    try {
      import_Log.default.debug(`designModel: Starting preview generation for ${modelId}`);
      const serverManager = this.ensureServerManager();
      serverManager.features = import_ServerManager2.ServerManagerFeatures.all;
      const httpServer = serverManager.ensureHttpServer(this._previewServerPort);
      const renderToken = Math.random().toString(36).substring(2, 15) + Date.now().toString(36);
      const geometryPath = `/temp/designmodel-geometry-${renderToken}.json`;
      const texturePath = `/temp/designmodel-texture-${renderToken}.png`;
      httpServer.registerTempContent(geometryPath, geometryJson, "application/json");
      if (textureDataUrl) {
        const base64Match = textureDataUrl.match(/^data:image\/png;base64,(.*)$/);
        if (base64Match) {
          const binaryData = Buffer.from(base64Match[1], "base64");
          httpServer.registerTempContent(texturePath, binaryData, "image/png");
        }
      }
      const baseUrl = `http://localhost:${httpServer.port}`;
      if (!httpServer.isListening) {
        await httpServer.waitForReady();
      }
      if (!this._cachedRenderer) {
        this._cachedRenderer = new import_PlaywrightPageRenderer.default(baseUrl);
        const initialized = await this._cachedRenderer.initialize();
        if (!initialized) {
          httpServer.clearTempContent();
          throw new Error("Could not initialize headless renderer");
        }
        await this._cachedRenderer.warmUp();
      }
      if (!this._cachedRenderer.isBrowserReady()) {
        this._cachedRenderer = new import_PlaywrightPageRenderer.default(baseUrl);
        const reinitialized = await this._cachedRenderer.initialize();
        if (!reinitialized) {
          httpServer.clearTempContent();
          throw new Error("Browser disconnected and could not be reinitialized");
        }
        await this._cachedRenderer.warmUp();
      }
      const renderer = this._cachedRenderer;
      let modelViewerUrl = `/?mode=modelviewer&geometry=${encodeURIComponent(geometryPath)}&skipVanilla=true`;
      if (textureDataUrl) {
        modelViewerUrl += `&texture=${encodeURIComponent(texturePath)}`;
      }
      const cameraX = 0.7;
      const cameraY = 0.5;
      const cameraZ = 0.7;
      const angleUrl = modelViewerUrl + `&cameraX=${cameraX}&cameraY=${cameraY}&cameraZ=${cameraZ}`;
      const result = await renderer.renderModelFast(angleUrl, {
        width: 768,
        height: 768,
        renderWaitTime: 2e3,
        canvasTimeout: 1e4,
        imageFormat: "png",
        forceNewContext: true
      });
      httpServer.clearTempContent();
      if (result.imageData) {
        previewImageData = result.imageData;
        import_Log.default.debug(`designModel: Preview image generated, size=${previewImageData.length} bytes`);
        if (geoProjectItem) {
          try {
            const designDef = await import_ModelDesignDefinition.default.ensureAsAccessoryOnProjectItem(geoProjectItem);
            if (designDef) {
              await designDef.savePreview(previewImageData);
            }
          } catch (e) {
            import_Log.default.debug(`designModel: Failed to save preview to accessory folder: ${e}`);
          }
        }
      } else {
        previewError = result.error || "No image data returned from renderer";
        import_Log.default.debug(`designModel: No preview image - ${previewError}`);
      }
    } catch (e) {
      previewError = `Preview generation failed: ${e}`;
      import_Log.default.debug(`designModel: Preview failed with exception: ${e}`);
    }
    if (errors.length > 0 && filesWritten.length === 0) {
      return {
        content: [
          {
            type: "text",
            text: `Model creation failed:
${errors.map((e) => `  - ${e}`).join("\n")}`
          }
        ],
        isError: true
      };
    }
    let resultText = `\u2705 Created model "${modelId}" (${usage}):
`;
    for (const file of filesWritten) {
      resultText += `  \u2713 ${file}
`;
    }
    if (conversionResult.warnings.length > 0) {
      resultText += `
Warnings:
${conversionResult.warnings.map((w) => `  - ${w}`).join("\n")}`;
    }
    if (errors.length > 0) {
      resultText += `
Errors:
${errors.map((e) => `  - ${e}`).join("\n")}`;
    }
    if (previewError) {
      resultText += `
Preview: ${previewError}`;
    }
    resultText += wiringNote;
    const responseContent = [];
    let previewImagePath;
    if (previewImageData) {
      const rawBase64 = Buffer.from(previewImageData).toString("base64");
      const fitted = MinecraftMcpServer.ensureImageFitsContext(rawBase64, "image/png");
      responseContent.push({
        type: "image",
        data: fitted.base64,
        mimeType: fitted.mimeType
      });
      try {
        const tempDir = path.join(projectPath, ".mct", "previews");
        if (!fs.existsSync(tempDir)) {
          fs.mkdirSync(tempDir, { recursive: true });
        }
        const fileName = `designmodel-${modelId}-${Date.now()}.png`;
        previewImagePath = path.join(tempDir, fileName);
        fs.writeFileSync(previewImagePath, previewImageData);
        responseContent.push({
          type: "text",
          text: `[MCT_IMAGE_PATH:${previewImagePath}]

${resultText}`
        });
      } catch (saveErr) {
        responseContent.push({
          type: "text",
          text: resultText
        });
      }
    } else {
      responseContent.push({
        type: "text",
        text: resultText
      });
    }
    return {
      content: responseContent,
      structuredContent: {
        success: filesWritten.length > 0,
        filesWritten,
        modelId,
        usage,
        wiredTo: wireTarget,
        geometryJson: conversionResult.geometry,
        warnings: conversionResult.warnings,
        errors,
        // Include preview status for debugging
        previewStatus: previewImagePath ? "generated" : previewError || "no-preview",
        // Include preview image path for Electron app (SDK filters out large base64 data)
        previewImagePath
      }
    };
  }
  /**
   * Unified tool for building structures in Minecraft projects.
   * Combines structure preview + export + project integration into one step.
   */
  async _designStructureOp(args) {
    if (!this._creatorTools || !this._env) {
      return {
        content: [{ type: "text", text: "Error: Creator Tools is not initialized" }],
        isError: true
      };
    }
    const blockVolume = args.blockVolume;
    if (!blockVolume) {
      return {
        content: [{ type: "text", text: "Error: blockVolume is required" }],
        isError: true
      };
    }
    const structureId = args.structureId;
    if (!structureId) {
      return {
        content: [{ type: "text", text: "Error: structureId is required" }],
        isError: true
      };
    }
    const projectPath = args.projectPath;
    if (!projectPath) {
      return {
        content: [{ type: "text", text: "Error: projectPath is required" }],
        isError: true
      };
    }
    if (!blockVolume.blockLayersBottomToTop || !blockVolume.key) {
      return {
        content: [{ type: "text", text: "Error: blockVolume must have blockLayersBottomToTop and key properties" }],
        isError: true
      };
    }
    const dimensionError = this._validateBlockVolumeDimensions(blockVolume);
    if (dimensionError) {
      return {
        content: [{ type: "text", text: `Error: ${dimensionError}` }],
        isError: true
      };
    }
    const effectiveSize = import_StructureUtilities.default.getEffectiveSize(blockVolume);
    const structure = import_StructureUtilities.default.createStructureFromIBlockVolume(blockVolume);
    const structureBytes = structure.getMCStructureBytes();
    if (!structureBytes) {
      return {
        content: [{ type: "text", text: "Error: Failed to generate MCStructure bytes from block volume" }],
        isError: true
      };
    }
    let project;
    try {
      const nodeStorage = new import_NodeStorage.default(projectPath, "");
      const projectFolder = nodeStorage.rootFolder;
      await projectFolder.ensureExists();
      project = new import_Project.default(this._creatorTools, path.basename(projectPath), null);
      project.setProjectFolder(projectFolder);
      await project.inferProjectItemsFromFilesRootFolder();
    } catch (e) {
      return {
        content: [{ type: "text", text: `Error: Failed to open project at ${projectPath}: ${e}` }],
        isError: true
      };
    }
    let bpFolder = await project.ensureDefaultBehaviorPackFolder();
    if (!bpFolder) {
      bpFolder = project.projectFolder?.ensureFolder("behavior_packs");
      if (!bpFolder) {
        return {
          content: [{ type: "text", text: "Error: Could not create behavior_packs folder" }],
          isError: true
        };
      }
      bpFolder = bpFolder.ensureFolder("main_bp");
      if (!bpFolder) {
        return {
          content: [{ type: "text", text: "Error: Could not create main_bp folder" }],
          isError: true
        };
      }
    }
    const structuresFolder = bpFolder.ensureFolder("structures");
    await structuresFolder.ensureExists();
    const structureFile = structuresFolder.ensureFile(`${structureId}.mcstructure`);
    structureFile.setContent(structureBytes);
    await structureFile.saveContent();
    const filesWritten = [];
    filesWritten.push(structureFile.fullPath);
    await project.inferProjectItemsFromFilesRootFolder();
    const structureProjectItem = project.items.find(
      (item) => item.projectPath && item.projectPath.endsWith(`${structureId}.mcstructure`)
    );
    if (structureProjectItem) {
      try {
        const designDef = await import_StructureDesignDefinition.default.ensureAsAccessoryOnProjectItem(structureProjectItem);
        if (designDef) {
          await designDef.updateDesign(blockVolume);
          import_Log.default.debug(`designStructure: Saved design to accessory folder for ${structureId}`);
        }
      } catch (e) {
        import_Log.default.debug(`designStructure: Failed to save design to accessory folder: ${e}`);
      }
    }
    let previewImageData;
    let previewError;
    try {
      const serverManager = this.ensureServerManager();
      serverManager.features = import_ServerManager2.ServerManagerFeatures.all;
      const httpServer = serverManager.ensureHttpServer(this._previewServerPort);
      const renderToken = Math.random().toString(36).substring(2, 15) + Date.now().toString(36);
      const structurePath = `/temp/designstructure-${renderToken}.mcstructure`;
      httpServer.registerTempContent(structurePath, structureBytes, "application/octet-stream");
      const baseUrl = `http://localhost:${httpServer.port}`;
      if (!httpServer.isListening) {
        await httpServer.waitForReady();
      }
      if (!this._cachedRenderer) {
        this._cachedRenderer = new import_PlaywrightPageRenderer.default(baseUrl);
        const initialized = await this._cachedRenderer.initialize();
        if (!initialized) {
          httpServer.clearTempContent();
          throw new Error("Could not initialize headless renderer");
        }
        await this._cachedRenderer.warmUp();
      }
      if (!this._cachedRenderer.isBrowserReady()) {
        this._cachedRenderer = new import_PlaywrightPageRenderer.default(baseUrl);
        const reinitialized = await this._cachedRenderer.initialize();
        if (!reinitialized) {
          httpServer.clearTempContent();
          throw new Error("Browser disconnected and could not be reinitialized");
        }
        await this._cachedRenderer.warmUp();
      }
      const renderer = this._cachedRenderer;
      let structureViewerUrl = `/?mode=structureviewer&structure=${encodeURIComponent(structurePath)}&hideChrome=true`;
      structureViewerUrl += `&skipVanilla=false`;
      structureViewerUrl += `&contentRoot=${encodeURIComponent("https://mctools.dev/")}`;
      const cameraX = 0.7;
      const cameraY = 0.5;
      const cameraZ = 0.7;
      const angleUrl = structureViewerUrl + `&cameraX=${cameraX}&cameraY=${cameraY}&cameraZ=${cameraZ}`;
      const result = await renderer.renderModelFast(angleUrl, {
        width: 800,
        height: 600,
        renderWaitTime: 3e3,
        canvasTimeout: 15e3,
        imageFormat: "png",
        forceNewContext: true
      });
      httpServer.clearTempContent();
      if (result.imageData) {
        previewImageData = result.imageData;
        if (structureProjectItem) {
          try {
            const designDef = await import_StructureDesignDefinition.default.ensureAsAccessoryOnProjectItem(structureProjectItem);
            if (designDef) {
              await designDef.savePreview(previewImageData);
            }
          } catch (e) {
            import_Log.default.debug(`designStructure: Failed to save preview to accessory folder: ${e}`);
          }
        }
      } else {
        previewError = result.error || "No image data returned from renderer";
      }
    } catch (e) {
      previewError = `Preview generation failed: ${e}`;
    }
    let resultText = `\u2705 Built structure "${structureId}":
`;
    for (const file of filesWritten) {
      resultText += `  \u2713 ${file}
`;
    }
    resultText += `
  Size: ${effectiveSize.x}x${effectiveSize.y}x${effectiveSize.z} blocks`;
    resultText += `
  Origin: (${blockVolume.southWestBottom.x}, ${blockVolume.southWestBottom.y}, ${blockVolume.southWestBottom.z})`;
    resultText += `
  Block types: ${Object.keys(blockVolume.key).length}`;
    if (previewError) {
      resultText += `

Preview: ${previewError}`;
    }
    const responseContent = [];
    if (previewImageData) {
      const rawBase64 = Buffer.from(previewImageData).toString("base64");
      const fitted = MinecraftMcpServer.ensureImageFitsContext(rawBase64, "image/png");
      responseContent.push({
        type: "image",
        data: fitted.base64,
        mimeType: fitted.mimeType
      });
      try {
        const tempDir = path.join(projectPath, ".mct", "previews");
        if (!fs.existsSync(tempDir)) {
          fs.mkdirSync(tempDir, { recursive: true });
        }
        const fileName = `designstructure-${structureId}-${Date.now()}.png`;
        const tempImagePath = path.join(tempDir, fileName);
        fs.writeFileSync(tempImagePath, previewImageData);
        responseContent.push({
          type: "text",
          text: `[MCT_IMAGE_PATH:${tempImagePath}]

${resultText}`
        });
      } catch (saveErr) {
        responseContent.push({
          type: "text",
          text: resultText
        });
      }
    } else {
      responseContent.push({
        type: "text",
        text: resultText
      });
    }
    return {
      content: responseContent,
      structuredContent: {
        success: filesWritten.length > 0,
        filesWritten,
        structureId,
        size: effectiveSize,
        origin: blockVolume.southWestBottom,
        blockTypeCount: Object.keys(blockVolume.key).length
      }
    };
  }
  /**
   * Returns starter model templates for common Minecraft entity types.
   * These provide proper Minecraft-scale geometry with blocky pixel-art style textures.
   */
  async _getModelTemplatesOp(args) {
    import_Log.default.message(
      `[MCP] Tool getModelTemplates called with args: ${JSON.stringify(args)}`
    );
    const template = await (0, import_ModelDesignTemplates.getModelTemplateAsync)(args.templateType);
    if (!template) {
      return {
        content: [
          {
            type: "text",
            text: `Unknown template type: ${args.templateType}. Available types: ${(0, import_ModelDesignTemplates.getAvailableTemplateTypes)().join(
              ", "
            )}`
          }
        ],
        isError: true
      };
    }
    return {
      content: [
        {
          type: "text",
          text: `Template: ${args.templateType}

${template.description}

This template can be passed directly to designModel.
Customize the colors in the 'textures' dictionary and adjust cube dimensions as needed.

Template JSON:
\`\`\`json
` + JSON.stringify(template, null, 2) + "\n```"
        }
      ],
      structuredContent: {
        template,
        templateType: args.templateType,
        description: template.description
      }
    };
  }
  /**
   * Validates an IBlockVolume has the required basic structure.
   * Size is now optional - if not provided, it will be inferred from the data.
   * String lengths and row counts don't need to match exactly - shorter strings
   * and missing rows are treated as air blocks.
   *
   * Returns an error message if validation fails, or undefined if valid.
   */
  _validateBlockVolumeDimensions(blockVolume) {
    const layers = blockVolume.blockLayersBottomToTop;
    if (!layers || layers.length === 0) {
      return "blockLayersBottomToTop must have at least one layer";
    }
    let hasContent = false;
    for (const layer of layers) {
      if (layer && layer.length > 0) {
        for (const row of layer) {
          if (row && row.length > 0) {
            hasContent = true;
            break;
          }
        }
        if (hasContent) break;
      }
    }
    if (!hasContent) {
      return "blockLayersBottomToTop must have at least one non-empty row";
    }
    return void 0;
  }
  /**
   * Preview a structure design (IBlockVolume) by converting it to an MCStructure and rendering a preview image.
   * Returns the preview as a base64-encoded PNG image from multiple angles.
   */
  async _previewStructureDesignOp(args) {
    if (!this._creatorTools || !this._env) {
      return {
        content: [{ type: "text", text: "Error: Creator Tools is not initialized" }],
        isError: true
      };
    }
    const blockVolume = args.blockVolume;
    if (!blockVolume) {
      return {
        content: [{ type: "text", text: "Error: blockVolume is required" }],
        isError: true
      };
    }
    if (!blockVolume.blockLayersBottomToTop || !blockVolume.key) {
      return {
        content: [{ type: "text", text: "Error: blockVolume must have blockLayersBottomToTop and key properties" }],
        isError: true
      };
    }
    const dimensionError = this._validateBlockVolumeDimensions(blockVolume);
    if (dimensionError) {
      return {
        content: [{ type: "text", text: `Error: ${dimensionError}` }],
        isError: true
      };
    }
    const effectiveSize = import_StructureUtilities.default.getEffectiveSize(blockVolume);
    const structure = import_StructureUtilities.default.createStructureFromIBlockVolume(blockVolume);
    const structureBytes = structure.getMCStructureBytes();
    if (!structureBytes) {
      return {
        content: [{ type: "text", text: "Error: Failed to generate MCStructure bytes from block volume" }],
        isError: true
      };
    }
    const serverManager = this.ensureServerManager();
    serverManager.features = import_ServerManager2.ServerManagerFeatures.all;
    const httpServer = serverManager.ensureHttpServer(this._previewServerPort);
    const renderToken = Math.random().toString(36).substring(2, 15) + Date.now().toString(36);
    const structurePath = `/temp/preview-structure-${renderToken}.mcstructure`;
    httpServer.registerTempContent(structurePath, structureBytes, "application/octet-stream");
    const baseUrl = `http://localhost:${httpServer.port}`;
    if (!httpServer.isListening) {
      await httpServer.waitForReady();
    }
    if (!httpServer.isListening) {
      import_Log.default.debugAlert(`HTTP server on port ${httpServer.port} is not actually listening. Server may have crashed.`);
      return {
        content: [
          {
            type: "text",
            text: `Error: Internal HTTP server on port ${httpServer.port} is not responding. Please restart the MCP server.`
          }
        ],
        isError: true
      };
    }
    if (!this._cachedRenderer) {
      this._cachedRenderer = new import_PlaywrightPageRenderer.default(baseUrl);
      const initialized = await this._cachedRenderer.initialize();
      if (!initialized) {
        httpServer.clearTempContent();
        this._cachedRenderer = void 0;
        return {
          content: [
            {
              type: "text",
              text: "Error: Could not initialize headless renderer. Ensure Chrome/Edge is available."
            }
          ],
          isError: true
        };
      }
      await this._cachedRenderer.warmUp();
    }
    if (!this._cachedRenderer.isBrowserReady()) {
      console.warn("Browser disconnected, reinitializing for structure preview...");
      this._cachedRenderer = new import_PlaywrightPageRenderer.default(baseUrl);
      const reinitialized = await this._cachedRenderer.initialize();
      if (!reinitialized) {
        httpServer.clearTempContent();
        this._cachedRenderer = void 0;
        return {
          content: [{ type: "text", text: "Error: Browser disconnected and could not be reinitialized." }],
          isError: true
        };
      }
      await this._cachedRenderer.warmUp();
    }
    const renderer = this._cachedRenderer;
    const width = args.width || 800;
    const height = args.height || 600;
    const multiAngle = args.multiAngle !== false;
    const imageFormat = args.imageFormat || "png";
    const jpegQuality = args.jpegQuality || 80;
    let structureViewerUrl = `/?mode=structureviewer&structure=${encodeURIComponent(structurePath)}&hideChrome=true`;
    structureViewerUrl += `&skipVanilla=false`;
    structureViewerUrl += `&contentRoot=${encodeURIComponent("https://mctools.dev/")}`;
    const sizeX = effectiveSize.x;
    const sizeY = effectiveSize.y;
    const sizeZ = effectiveSize.z;
    let result;
    if (multiAngle) {
      const maxDim = Math.max(sizeX, sizeY, sizeZ);
      const radius = maxDim * 1.5 + 5;
      const centerX = sizeX / 2;
      const centerY = sizeY / 2;
      const centerZ = sizeZ / 2;
      const anglePresets = [
        {
          key: "front-right",
          x: centerX + radius * 0.7,
          y: centerY + radius * 0.5,
          z: centerZ + radius * 0.7,
          label: "Front Right"
        },
        {
          key: "front-left",
          x: centerX - radius * 0.7,
          y: centerY + radius * 0.5,
          z: centerZ + radius * 0.7,
          label: "Front Left"
        },
        {
          key: "back-right",
          x: centerX + radius * 0.7,
          y: centerY + radius * 0.5,
          z: centerZ - radius * 0.7,
          label: "Back Right"
        },
        {
          key: "back-left",
          x: centerX - radius * 0.7,
          y: centerY + radius * 0.5,
          z: centerZ - radius * 0.7,
          label: "Back Left"
        },
        {
          key: "top-down",
          x: centerX,
          y: centerY + radius * 1.2,
          z: centerZ + 0.1,
          // Slight offset to avoid looking straight down
          label: "Top Down"
        },
        {
          key: "side-right",
          x: centerX + radius,
          y: centerY + radius * 0.3,
          z: centerZ,
          label: "Side Right"
        },
        {
          key: "side-left",
          x: centerX - radius,
          y: centerY + radius * 0.3,
          z: centerZ,
          label: "Side Left"
        }
      ];
      const requestedAngles = args.anglePresets || ["front-right", "back-left"];
      const anglesToRender = requestedAngles.map((a) => anglePresets.find((p) => p.key === a.toLowerCase())).filter(Boolean);
      if (anglesToRender.length === 0) {
        anglesToRender.push(anglePresets[0], anglePresets[3]);
      }
      const cols = anglesToRender.length >= 4 ? 2 : anglesToRender.length;
      const rows = anglesToRender.length >= 4 ? 2 : 1;
      const renderedImages = [];
      const renderErrors = [];
      const blockCount = sizeX * sizeY * sizeZ;
      const baseCanvasTimeout = 8e3;
      const additionalCanvasTimeout = Math.min(blockCount / 500, 15e3);
      const canvasTimeout = Math.round(baseCanvasTimeout + additionalCanvasTimeout);
      const baseWaitTime = 2500;
      const additionalWaitTime = Math.min(blockCount / 250, 6e3);
      const renderWaitTime = Math.round(baseWaitTime + additionalWaitTime);
      for (let i = 0; i < anglesToRender.length; i++) {
        const angle = anglesToRender[i];
        const imgWidth = Math.floor(width / cols);
        const imgHeight = Math.floor(height / rows);
        const angleUrl = structureViewerUrl + `&cameraX=${angle.x}&cameraY=${angle.y}&cameraZ=${angle.z}&t=${Date.now()}_${i}`;
        const angleResult = await renderer.renderModelFast(angleUrl, {
          width: imgWidth,
          height: imgHeight,
          renderWaitTime,
          canvasTimeout,
          imageFormat: "png",
          forceNewContext: true
          // Force new context for each angle to ensure camera position is applied
        });
        if (angleResult.imageData) {
          renderedImages.push({ label: angle.label, imageData: angleResult.imageData });
        } else if (angleResult.error) {
          renderErrors.push(`${angle.label}: ${angleResult.error}`);
        }
      }
      if (renderedImages.length > 0) {
        const stitchedImage = await import_ImageGenerationUtilities.default.stitchImagesWithLabels(
          renderedImages,
          Math.floor(width / cols),
          Math.floor(height / rows),
          cols,
          rows
        );
        result = { imageData: stitchedImage };
      } else {
        let errorMsg = `No images rendered for multi-angle view. `;
        errorMsg += `Structure size: ${sizeX}x${sizeY}x${sizeZ} (${blockCount} blocks). `;
        errorMsg += `Requested angles: ${requestedAngles.join(", ")}. `;
        errorMsg += `Valid angles found: ${anglesToRender.length}. `;
        if (renderErrors.length > 0) {
          errorMsg += `Render errors: ${renderErrors.join("; ")}`;
        }
        result = { imageData: void 0, error: errorMsg };
      }
    } else {
      const blockCount = sizeX * sizeY * sizeZ;
      const baseCanvasTimeout = 8e3;
      const additionalCanvasTimeout = Math.min(blockCount / 500, 15e3);
      const canvasTimeout = Math.round(baseCanvasTimeout + additionalCanvasTimeout);
      const baseWaitTime = 2500;
      const additionalWaitTime = Math.min(blockCount / 250, 6e3);
      const renderWaitTime = Math.round(baseWaitTime + additionalWaitTime);
      result = await renderer.renderModelFast(structureViewerUrl, {
        width,
        height,
        renderWaitTime,
        canvasTimeout,
        imageFormat,
        jpegQuality,
        forceNewContext: true
      });
    }
    httpServer.clearTempContent();
    if (result.error) {
      return {
        content: [{ type: "text", text: `Rendering failed: ${result.error}` }],
        isError: true
      };
    }
    if (result.imageData) {
      let finalImageData = result.imageData;
      if (imageFormat === "jpeg" && multiAngle) {
        finalImageData = await import_ImageGenerationUtilities.default.convertPngToJpeg(finalImageData, jpegQuality);
      }
      const rawBase64Image = Buffer.from(finalImageData).toString("base64");
      const structOutputMime = imageFormat === "jpeg" ? "image/jpeg" : "image/png";
      const fitted = MinecraftMcpServer.ensureImageFitsContext(rawBase64Image, structOutputMime);
      return {
        content: [
          {
            type: "image",
            data: fitted.base64,
            mimeType: fitted.mimeType
          },
          {
            type: "text",
            text: `Structure preview generated: ${effectiveSize.x}x${effectiveSize.y}x${effectiveSize.z} blocks`
          }
        ],
        structuredContent: {
          size: effectiveSize,
          origin: blockVolume.southWestBottom,
          blockTypeCount: Object.keys(blockVolume.key).length
        }
      };
    }
    return {
      content: [{ type: "text", text: "Error: No image data returned from renderer" }],
      isError: true
    };
  }
  /**
   * Export a structure design (IBlockVolume) to an MCStructure file.
   */
  async _exportStructureDesignOp(args) {
    const blockVolume = args.blockVolume;
    const outputPath = args.outputPath;
    const overwrite = args.overwrite;
    if (!blockVolume) {
      return {
        content: [{ type: "text", text: "Error: blockVolume is required" }],
        isError: true
      };
    }
    if (!outputPath) {
      return {
        content: [{ type: "text", text: "Error: outputPath is required" }],
        isError: true
      };
    }
    if (!blockVolume.blockLayersBottomToTop || !blockVolume.key) {
      return {
        content: [{ type: "text", text: "Error: blockVolume must have blockLayersBottomToTop and key properties" }],
        isError: true
      };
    }
    const dimensionError = this._validateBlockVolumeDimensions(blockVolume);
    if (dimensionError) {
      return {
        content: [{ type: "text", text: `Error: ${dimensionError}` }],
        isError: true
      };
    }
    const effectiveSize = import_StructureUtilities.default.getEffectiveSize(blockVolume);
    if (!overwrite && fs.existsSync(outputPath)) {
      return {
        content: [
          {
            type: "text",
            text: `Error: File already exists: ${outputPath}
Use overwrite: true to replace it.`
          }
        ],
        isError: true
      };
    }
    const structure = import_StructureUtilities.default.createStructureFromIBlockVolume(blockVolume);
    const structureBytes = structure.getMCStructureBytes();
    if (!structureBytes) {
      return {
        content: [{ type: "text", text: "Error: Failed to generate MCStructure bytes from block volume" }],
        isError: true
      };
    }
    try {
      const outputDir = path.dirname(outputPath);
      if (!fs.existsSync(outputDir)) {
        fs.mkdirSync(outputDir, { recursive: true });
      }
      fs.writeFileSync(outputPath, Buffer.from(structureBytes));
    } catch (e) {
      return {
        content: [{ type: "text", text: `Error: Failed to write file: ${e}` }],
        isError: true
      };
    }
    const resultText = `Successfully exported structure:
  \u2713 ${outputPath}
  Size: ${effectiveSize.x}x${effectiveSize.y}x${effectiveSize.z} blocks
  Origin: (${blockVolume.southWestBottom.x}, ${blockVolume.southWestBottom.y}, ${blockVolume.southWestBottom.z})
  Block types: ${Object.keys(blockVolume.key).length}`;
    return {
      content: [{ type: "text", text: resultText }],
      structuredContent: {
        success: true,
        filePath: outputPath,
        size: effectiveSize,
        origin: blockVolume.southWestBottom,
        blockTypeCount: Object.keys(blockVolume.key).length
      }
    };
  }
  /**
   * Configure MCP prompts that expose server configuration to AI assistants.
   * The main prompt is "working-folder" which tells the AI where to write content.
   */
  _configurePrompts() {
    this._server.prompt("working-folder", async () => {
      if (this._workingFolder) {
        return {
          messages: [
            {
              role: "user",
              content: {
                type: "text",
                text: `The working folder for Minecraft content is: ${this._workingFolder}

When creating Minecraft content, use this folder as the base path for all file operations. For example:
- Create projects at: ${this._workingFolder}
- Write model designs to: ${this._workingFolder}
- Save structures to: ${this._workingFolder}

Always use absolute paths based on this working folder.`
              }
            }
          ]
        };
      } else {
        return {
          messages: [
            {
              role: "user",
              content: {
                type: "text",
                text: "No working folder has been set. The MCP server was started without the -i argument. You should ask the user where they want to create Minecraft content, or use the current working directory."
              }
            }
          ]
        };
      }
    });
    this._server.resource("working-folder-config", "config://working-folder", async (uri) => {
      return {
        contents: [
          {
            uri: uri.href,
            mimeType: "application/json",
            text: JSON.stringify(
              {
                workingFolder: this._workingFolder || null,
                hasWorkingFolder: !!this._workingFolder,
                message: this._workingFolder ? `Use ${this._workingFolder} as the base path for Minecraft content operations.` : "No working folder set. Use current directory or ask user for path."
              },
              null,
              2
            )
          }
        ]
      };
    });
  }
  _configureTools() {
    const infoSetForm = import_Database.default.ensureFormLoadedSync("info_set", "info");
    const stateForm = import_Database.default.ensureFormLoadedSync("action_set", "world_state");
    const actionSetForm = import_Database.default.ensureFormLoadedSync("action_set", "action_group");
    if (infoSetForm && actionSetForm && stateForm) {
      const validationResultForm = { info: import_DataFormZod.default.getZodSchema(infoSetForm) };
      const actionSetResultForm = { actionSet: import_DataFormZod.default.getZodSchema(actionSetForm) };
      const stateResultForm = { state: import_DataFormZod.default.getZodSchema(stateForm) };
      this._server.registerTool(
        "createProject",
        {
          title: "Creates scaffolding and files for a new Minecraft project",
          description: "Creates all of the foundational files for a new Minecraft project.",
          inputSchema: {
            folderPathToCreateProjectAt: import_zod.z.string(),
            title: import_zod.z.string(),
            description: import_zod.z.string().optional(),
            newName: import_zod.z.string(),
            creator: import_zod.z.string(),
            template: import_zod.z.enum([
              "addonStarter",
              "tsStarter",
              "addonFull",
              "scriptBox",
              "dlStarter",
              "editor-scriptBox",
              "editor-basics"
            ])
          }
        },
        this._createOp
      );
      this._server.registerTool(
        "addItem",
        {
          title: "Adds files for Minecraft items to a project",
          description: "Adds files for Minecraft items to a project.",
          inputSchema: {
            folderPathToCreateProjectAt: import_zod.z.string(),
            templateType: import_zod.z.enum([
              "basicUnitCubeBlock",
              "crateBlock",
              "basicDieBlock",
              "sushiRollBlock",
              "fishBowlBlock",
              "hardBiscuit",
              "pear",
              "elixir",
              "rod",
              "key",
              "customSword",
              "wrench",
              "allay",
              "axolotl",
              "cat",
              "cow",
              "creeper",
              "enderman",
              "rabbit",
              "pig",
              "sheep",
              "skeleton",
              "wolf",
              "zombie",
              "spawn_rule",
              "loot_table",
              "recipe_shapeless",
              "recipe_shaped",
              "feature_rule",
              "jigsaw",
              "atmospherics",
              "color_grading",
              "lighting",
              "pbr",
              "biome_behavior",
              "entity_behavior",
              "entity_resources",
              "item_behavior",
              "attachable",
              "block_behavior",
              "block_culling",
              "block_catalog",
              "biome_resource",
              "aggregate_feature",
              "animation",
              "animation_controller",
              "render_controller"
            ]),
            name: import_zod.z.string()
          }
        },
        this._addOp
      );
      this._server.registerTool(
        "createMinecraftContent",
        {
          title: "Create Minecraft content from simplified meta-schema",
          description: "Creates Minecraft Bedrock content (entities, blocks, items, etc.) from a simplified, AI-friendly meta-schema. The schema supports three levels of abstraction: traits (pre-packaged bundles), simplified properties, and native components. A single definition generates all required behavior pack and resource pack files including manifests, entity definitions, block definitions, item definitions, loot tables, recipes, spawn rules, and features.",
          inputSchema: {
            definition: import_ContentMetaSchemaZod.MinecraftContentSchema,
            outputPath: import_zod.z.string().describe("Absolute path to the folder where content should be generated")
          }
        },
        this._createMinecraftContentOp
      );
      this._server.registerTool(
        "getEffectiveContentSchema",
        {
          title: "Get effective content schema for a Minecraft project",
          description: "Analyzes an existing Minecraft Bedrock project and infers what traits and simplified properties would represent it in the meta-schema format. This is useful for understanding existing content, importing legacy projects into the simplified format, or generating documentation. Returns an IMinecraftContentDefinition that could be used with createMinecraftContent to recreate similar content.",
          inputSchema: {
            folderPath: import_zod.z.string().describe("Absolute path to the project folder to analyze"),
            options: import_zod.z.object({
              minTraitConfidence: import_zod.z.number().min(0).max(1).optional().describe("Minimum confidence (0-1) for trait detection. Default: 0.6"),
              includeRawComponents: import_zod.z.boolean().optional().describe("Include raw components not explained by traits. Default: true"),
              inferNamespace: import_zod.z.boolean().optional().describe("Try to detect namespace from identifiers. Default: true"),
              includeBehaviorPresets: import_zod.z.boolean().optional().describe("Include behavior presets in addition to traits. Default: true"),
              includeComponentGroups: import_zod.z.boolean().optional().describe("Include component groups as raw data. Default: false"),
              includeEvents: import_zod.z.boolean().optional().describe("Include events as raw data. Default: false")
            }).optional().describe("Options for controlling inference behavior")
          }
        },
        this._getEffectiveContentSchemaOp
      );
      this._server.registerTool(
        "validateContent",
        {
          title: "Validate Minecraft Bedrock content",
          description: "Validates content (either single JSON files or a ZIP file encoded as a base64 string) and returns validation results.",
          inputSchema: { jsonContentOrBase64ZipContent: import_zod.z.string() },
          outputSchema: validationResultForm
        },
        this._processValidateContent
      );
      this._server.registerTool(
        "validateFile",
        {
          title: "Validate Minecraft Bedrock at a file path",
          description: "Validates content at a particular file path",
          inputSchema: { filePath: import_zod.z.string() },
          outputSchema: validationResultForm
        },
        this._processValidateContentAtPath
      );
      this._server.registerTool(
        "createMinecraftSessionWithContent",
        {
          title: "Starts a new Minecraft session",
          description: "Starts a new Minecraft session using the specified content, which can be used for validating content and gameplay.",
          inputSchema: {
            sessionName: import_zod.z.string(),
            packagedMcaddonOrMcworldFilePath: import_zod.z.string(),
            testPlayerNameToUse: import_zod.z.string()
          },
          outputSchema: stateResultForm
        },
        this._createMinecraftSession
      );
      this._server.registerTool(
        "moveSessionPlayerToLocation",
        {
          title: "Moves a Minecraft player to a specific location in the Minecraft session",
          description: "Starts a new Minecraft session using the specified content, which can be used for validating content and gameplay.",
          inputSchema: {
            sessionName: import_zod.z.string(),
            playerName: import_zod.z.string(),
            locationToHavePlayerMoveTo: import_zod.z.object({ x: import_zod.z.number(), y: import_zod.z.number(), z: import_zod.z.number() })
          },
          outputSchema: stateResultForm
        },
        this._moveSessionPlayerToLocation
      );
      this._server.registerTool(
        "runCommandInMinecraft",
        {
          title: "Runs a command in the specified Minecraft session",
          description: "Runs a command in the specified Minecraft session.",
          inputSchema: { sessionName: import_zod.z.string(), command: import_zod.z.string() }
        },
        this._runCommandOp
      );
      this._server.registerTool(
        "runActionSetInMinecraft",
        {
          title: "Runs a set of actions in the specified Minecraft session and returns a resulting state.",
          description: "Runs a set of actions in the specified Minecraft session.",
          inputSchema: actionSetResultForm
        },
        this._runActionSetOp
      );
    }
    this._server.registerTool(
      "readImageFile",
      {
        title: "Read an image file from the file system",
        description: "Reads an image file and returns it as base64-encoded data that can be displayed. Supports image formats like .png, .jpg, .gif, .webp, .bmp, .tiff, etc. Useful for viewing screenshots, test output images, texture files, and other image content generated by tests or builds.",
        inputSchema: {
          filePath: import_zod.z.string().describe("The absolute path to the image file to read")
        }
      },
      this._readImageFileOp
    );
    this._server.registerTool(
      "writeImageFile",
      {
        title: "Write an image file to the file system from base64 data",
        description: "Writes base64-encoded image data to a file and returns the resulting image for verification. Supports image formats like .png, .jpg, .webp, .bmp, .tiff, etc. The parent directory will be created if it doesn't exist. The tool returns the written image so you can verify the output.",
        inputSchema: {
          filePath: import_zod.z.string().describe("The absolute path where the image file should be written"),
          base64Data: import_zod.z.string().describe("Base64-encoded image data to write."),
          mimeType: import_zod.z.string().optional().describe(
            "Optional MIME type (e.g., 'image/png'). If not provided, it will be inferred from the file extension."
          )
        }
      },
      this._writeImageFileFromBase64Op
    );
    this._server.registerTool(
      "writeImageFileFromSvg",
      {
        title: "Convert SVG to PNG and write to file",
        description: "Converts raw SVG markup to PNG and writes it to a file. Returns the resulting image for verification. You can optionally specify width/height for the output PNG (defaults to SVG's intrinsic size). The parent directory will be created if it doesn't exist.",
        inputSchema: {
          filePath: import_zod.z.string().describe("The absolute path where the PNG file should be written"),
          svgContent: import_zod.z.string().describe("Raw SVG markup text to convert to PNG."),
          width: import_zod.z.number().optional().describe("Optional width in pixels for the output PNG. Defaults to SVG's intrinsic width."),
          height: import_zod.z.number().optional().describe("Optional height in pixels for the output PNG. Defaults to SVG's intrinsic height.")
        }
      },
      this._writeImageFileFromSvgOp
    );
    const pixelColorSchema = import_zod.z.object({
      r: import_zod.z.number().optional().describe("Red component 0-255"),
      g: import_zod.z.number().optional().describe("Green component 0-255"),
      b: import_zod.z.number().optional().describe("Blue component 0-255"),
      a: import_zod.z.number().optional().describe("Alpha component 0-255 (default: 255 = opaque)"),
      hex: import_zod.z.string().optional().describe("Hex color string like '#FF0000' or '#FF0000FF' (with alpha)")
    }).describe("Color: use either r/g/b values or hex string");
    this._server.registerTool(
      "writeImageFileFromPixelArt",
      {
        title: "Create a PNG image from pixel art definition",
        description: "Creates a PNG image from a pixel art definition using ASCII-style lines and a color palette. Each character in the lines array maps to a color in the palette. Space characters are transparent. This is ideal for creating Minecraft-style pixel art textures. Example: lines=['XXX', 'X X', 'XXX'] with palette={'X': {hex: '#FF0000'}} creates a red square with transparent center. Use scale to increase pixel size (scale=2 makes each character 2x2 pixels). The parent directory will be created if it doesn't exist.",
        inputSchema: {
          filePath: import_zod.z.string().describe("The absolute path where the PNG file should be written (must end in .png)"),
          lines: import_zod.z.array(import_zod.z.string()).describe(
            "Array of strings representing pixel rows from top to bottom. Each character is looked up in the palette. Space = transparent."
          ),
          palette: import_zod.z.record(import_zod.z.string(), pixelColorSchema).describe(
            "Map of characters to colors. Do not define space ' ' - it is reserved for transparency. Example: { 'X': { hex: '#FF0000' }, 'O': { r: 0, g: 255, b: 0 } }"
          ),
          scale: import_zod.z.number().optional().describe("Scale factor for output. Each character becomes scale\xD7scale pixels. Default: 1"),
          backgroundColor: pixelColorSchema.optional().describe("Optional background color. Default: transparent (alpha=0)")
        }
      },
      this._writeImageFileFromPixelArtOp
    );
    const mcpTexturedRectangleSchema = import_zod.z.object({
      type: import_zod.z.enum(["solid", "random_noise", "dither_noise", "perlin_noise", "stipple_noise", "gradient"]).describe(
        "Fill algorithm: 'solid' for flat color, 'stipple_noise' for organic textures (skin, stone), 'dither_noise' for structured patterns (metal, fabric), 'perlin_noise' for smooth organic (grass, water), 'random_noise' for rough textures (gravel), 'gradient' for smooth transitions"
      ),
      colors: import_zod.z.array(import_zod.z.string()).describe(
        "Colors to use. For 'solid', only first color is used. For noise types, provide 2+ colors for richer textures (hex like '#FF0000')"
      ),
      factor: import_zod.z.number().optional().describe("Noise intensity from 0 to 1. Higher = more variation. Default: 0.2. Ignored for 'solid'."),
      seed: import_zod.z.number().optional().describe("Random seed for deterministic results. Ignored for 'solid'."),
      pixelSize: import_zod.z.number().optional().describe("Size of noise pixels. Larger = blockier. Default: 1. Ignored for 'solid'."),
      scale: import_zod.z.number().optional().describe("For perlin_noise: controls smoothness. Larger = smoother. Default: 4.")
    }).describe(
      "Textured rectangle fill - use type:'solid' for flat colors, or noise types for Minecraft-style textures"
    );
    const mcpLightingEffectSchema = import_zod.z.object({
      preset: import_zod.z.enum(["inset", "outset", "pillow", "ambient_occlusion"]).describe(
        "Lighting preset: 'inset' = darken top/left, lighten bottom/right (recessed panels); 'outset' = lighten top/left, darken bottom/right (raised blocks); 'pillow' = darken edges, lighten center (rounded look); 'ambient_occlusion' = darken corners/edges (realistic depth)"
      ),
      intensity: import_zod.z.number().optional().describe("Effect intensity (0.0-1.0). Default: 0.3. Higher = more pronounced effect."),
      angle: import_zod.z.number().optional().describe(
        "Light source angle in degrees (0-360). Default: 315 (top-left). 0=right, 90=bottom, 180=left, 270=top."
      )
    }).describe("Lighting effect for pseudo-3D depth simulation");
    const mcpBorderSideSchema = import_zod.z.object({
      style: import_zod.z.enum(["solid", "dashed", "worn", "highlight"]).describe(
        "Border style: 'solid' = continuous line; 'dashed' = alternating segments; 'worn' = irregular/weathered edge; 'highlight' = bright/glowing edge"
      ),
      width: import_zod.z.number().optional().describe("Border width in pixels (1-8). Default: 1."),
      color: import_zod.z.string().optional().describe("Border color (hex). If not specified, auto-calculated from texture (darker or lighter).")
    }).describe("Configuration for a single border side");
    const mcpBorderEffectSchema = import_zod.z.object({
      all: mcpBorderSideSchema.optional().describe("Shorthand: apply this configuration to all sides. Individual side properties override this."),
      top: mcpBorderSideSchema.optional().describe("Top border configuration (overrides 'all')."),
      right: mcpBorderSideSchema.optional().describe("Right border configuration (overrides 'all')."),
      bottom: mcpBorderSideSchema.optional().describe("Bottom border configuration (overrides 'all')."),
      left: mcpBorderSideSchema.optional().describe("Left border configuration (overrides 'all')."),
      seed: import_zod.z.number().optional().describe("Random seed for 'worn' style deterministic weathering.")
    }).describe(
      "Border effect with CSS-like syntax. Use 'all' for uniform borders, or specify individual sides. Example: { all: { style: 'highlight', width: 1 } } or { top: { style: 'highlight' }, left: { style: 'highlight' } }"
    );
    const mcpOverlayEffectSchema = import_zod.z.object({
      pattern: import_zod.z.enum(["cracks", "scratches", "moss", "rust", "sparkle", "veins"]).describe(
        "Overlay pattern: 'cracks' = dark fracture lines; 'scratches' = light linear marks; 'moss' = green organic patches; 'rust' = orange/brown oxidation; 'sparkle' = bright highlight dots; 'veins' = dark branching lines"
      ),
      density: import_zod.z.number().optional().describe("Coverage density (0.0-1.0). Default: 0.3. Higher = more coverage."),
      color: import_zod.z.string().optional().describe("Override color for the pattern (hex). Uses pattern-appropriate default if not specified."),
      seed: import_zod.z.number().optional().describe("Random seed for deterministic pattern placement.")
    }).describe("Overlay effect for surface weathering and detail");
    const mcpColorVariationEffectSchema = import_zod.z.object({
      mode: import_zod.z.enum(["hue_shift", "saturation_jitter", "value_jitter", "palette_snap"]).describe(
        "Color variation mode: 'hue_shift' = randomly shift hue; 'saturation_jitter' = randomly vary saturation; 'value_jitter' = randomly vary brightness; 'palette_snap' = snap colors to nearest palette entry"
      ),
      amount: import_zod.z.number().optional().describe("Variation amount (0.0-1.0). Default: 0.1. Higher = more variation."),
      palette: import_zod.z.array(import_zod.z.string()).optional().describe("For 'palette_snap': array of colors to snap to (hex strings)."),
      seed: import_zod.z.number().optional().describe("Random seed for deterministic variation.")
    }).describe("Color variation effect to reduce flatness");
    const mcpTilingEffectSchema = import_zod.z.object({
      seamless: import_zod.z.boolean().optional().describe("Make edges seamless for repeating textures by blending edges."),
      pattern: import_zod.z.enum(["brick", "herringbone", "basketweave", "random"]).optional().describe("Tiling pattern for block arrangement (applied during generation, not post-process)."),
      offset: import_zod.z.number().optional().describe("Offset amount for brick pattern (0.0-1.0). Default: 0.5.")
    }).describe("Tiling effect for seamless and patterned textures");
    const mcpTextureEffectsSchema = import_zod.z.object({
      lighting: mcpLightingEffectSchema.optional().describe("Lighting effect for pseudo-3D depth."),
      border: mcpBorderEffectSchema.optional().describe("Border effect for edges/outlines (CSS-like syntax)."),
      overlay: import_zod.z.union([mcpOverlayEffectSchema, import_zod.z.array(mcpOverlayEffectSchema)]).optional().describe(
        "Overlay effect(s) for surface detail. Can be a single effect or array of effects applied in order."
      ),
      colorVariation: mcpColorVariationEffectSchema.optional().describe("Color variation effect."),
      tiling: mcpTilingEffectSchema.optional().describe("Tiling effect for seamless textures.")
    }).describe(
      "Post-processing effects applied to the texture. Order: colorVariation -> lighting -> overlay -> border -> tiling. Example: { lighting: { preset: 'inset', intensity: 0.3 }, border: { all: { style: 'highlight' } } }"
    );
    const mcpPixelColorSchema = import_zod.z.object({
      r: import_zod.z.number().optional().describe("Red channel (0-255)"),
      g: import_zod.z.number().optional().describe("Green channel (0-255)"),
      b: import_zod.z.number().optional().describe("Blue channel (0-255)"),
      a: import_zod.z.number().optional().describe("Alpha channel (0-255, default 255 = opaque)"),
      hex: import_zod.z.string().optional().describe("Alternative: hex color string like '#FF0000'")
    }).describe("Pixel color: use either r/g/b values or hex string");
    const mcpPixelArtSchema = import_zod.z.object({
      scaleMode: import_zod.z.enum(["unit", "exact", "cover"]).optional().describe(
        "How to scale the pixel art: 'unit' (default) = each character is 1 Minecraft unit, x/y are in units; 'exact' = each character is 1 pixel, x/y are in pixels; 'cover' = stretch to fill the face, x/y ignored"
      ),
      x: import_zod.z.number().optional().describe("X offset (in units for 'unit' mode, pixels for 'exact' mode, ignored for 'cover')"),
      y: import_zod.z.number().optional().describe("Y offset (in units for 'unit' mode, pixels for 'exact' mode, ignored for 'cover')"),
      lines: import_zod.z.array(import_zod.z.string()).describe("ASCII art lines where each character maps to a palette color. Space = transparent."),
      palette: import_zod.z.record(import_zod.z.string(), mcpPixelColorSchema).describe("Map of characters to colors. Do not define space ' ' - it is reserved for transparency.")
    }).describe(
      "Pixel art overlay using ASCII patterns. Default scaleMode='unit' means each character = 1 Minecraft unit. Example: { lines: ['E E', ' n '], palette: { E: { hex: '#000000' }, n: { hex: '#402020' } } }"
    );
    const mcpFaceContentSchema = import_zod.z.object({
      textureId: import_zod.z.string().optional().describe("Reference to a texture defined in the model's textures dictionary"),
      background: mcpTexturedRectangleSchema.optional().describe(
        "Background fill using a textured rectangle. Examples: { type: 'solid', colors: ['#808080'] } or { type: 'stipple_noise', colors: ['#8B8B8B', '#7A7A7A'], seed: 123 }"
      ),
      svg: import_zod.z.string().optional().describe("SVG content overlay (rendered on top of background)"),
      pixelArt: import_zod.z.array(mcpPixelArtSchema).optional().describe("Pixel art overlays rendered on top of background/texture. Layers are applied in order."),
      effects: mcpTextureEffectsSchema.optional().describe(
        "Post-processing effects applied after rendering. Example: { lighting: { preset: 'inset', intensity: 0.3 }, border: { all: { style: 'highlight' } } }"
      ),
      rotation: import_zod.z.number().optional().describe("Rotation in degrees (0, 90, 180, 270)")
    }).describe(
      "Face content: use textureId to reference a named texture, or specify background (preferred) for inline content"
    );
    const mcpCubeFacesSchema = import_zod.z.object({
      north: mcpFaceContentSchema.optional(),
      south: mcpFaceContentSchema.optional(),
      east: mcpFaceContentSchema.optional(),
      west: mcpFaceContentSchema.optional(),
      up: mcpFaceContentSchema.optional(),
      down: mcpFaceContentSchema.optional()
    });
    const mcpTextureDefinitionSchema = import_zod.z.object({
      background: mcpTexturedRectangleSchema.optional().describe(
        "Background fill for this texture. Examples: { type: 'solid', colors: ['#FF0000'] } or { type: 'stipple_noise', colors: ['#8B8B8B', '#7A7A7A'], seed: 123 }"
      ),
      svg: import_zod.z.string().optional().describe("SVG content overlay (rendered on top of background)"),
      pixelArt: import_zod.z.array(mcpPixelArtSchema).optional().describe("Pixel art overlays rendered on top of background. Layers are applied in order."),
      effects: mcpTextureEffectsSchema.optional().describe(
        "Post-processing effects applied after rendering. Example: { lighting: { preset: 'outset', intensity: 0.3 }, overlay: { pattern: 'cracks', density: 0.2 } }"
      )
    }).describe("A reusable texture definition with background fill and optional SVG/pixelArt overlay and effects");
    const vector3Schema = import_zod.z.array(import_zod.z.number()).min(3).max(3);
    const vector2Schema = import_zod.z.array(import_zod.z.number()).min(2).max(2);
    const mcpDesignCubeSchema = import_zod.z.object({
      origin: vector3Schema,
      size: vector3Schema,
      faces: mcpCubeFacesSchema,
      inflate: import_zod.z.number().optional(),
      mirror: import_zod.z.boolean().optional()
    });
    const mcpDesignBoneSchema = import_zod.z.object({
      name: import_zod.z.string(),
      parent: import_zod.z.string().optional(),
      pivot: vector3Schema.optional(),
      rotation: vector3Schema.optional(),
      cubes: import_zod.z.array(mcpDesignCubeSchema),
      mirror: import_zod.z.boolean().optional()
    });
    const mcpModelDesignSchema = import_zod.z.object({
      formatVersion: import_zod.z.string().optional(),
      identifier: import_zod.z.string().describe("Model identifier, e.g., 'custom_block' or 'geometry.custom_block'"),
      description: import_zod.z.string().optional(),
      textureSize: vector2Schema.optional().describe("Texture atlas size [width, height], default [64, 64]"),
      pixelsPerUnit: import_zod.z.number().optional().describe(
        "Pixels per Minecraft unit for texture generation. pixelsPerUnit \xD7 16 = pixels per block. Values: 1 (16px/block, vanilla), 2 (32px/block, default HD), 4 (64px/block, high-res)"
      ),
      textures: import_zod.z.record(import_zod.z.string(), mcpTextureDefinitionSchema).optional().describe(
        "Named texture definitions that can be referenced by faces using textureId. Define textures once here, then reference them in faces with { textureId: 'texture_name' }. This enables texture reuse across multiple faces and reduces token usage."
      ),
      visibleBoundsSize: vector3Schema.optional(),
      visibleBoundsOffset: vector3Schema.optional(),
      bones: import_zod.z.array(mcpDesignBoneSchema).describe("The bones that make up this model, each containing cubes")
    });
    this._server.registerTool(
      "designModel",
      {
        title: "Design a 3D model in a Minecraft project",
        description: "Designs a 3D model (geometry + texture) and saves it to a Minecraft project folder. Use this tool for iterative model design - it handles everything in one step:\n\n1. \u2705 Validates the model design\n2. \u2705 Creates geometry (.geo.json) and texture (.png) files\n3. \u2705 Saves files to the correct project location (auto-detects pack structure)\n4. \u2705 Persists the design for future iteration (update existing models)\n5. \u2705 Auto-wires to matching entity/block/item if found\n6. \u2705 Returns a preview image\n\n**Usage parameter:**\n- 'entity': Saves to /models/entity/ and /textures/entity/ (default)\n- 'block': Saves to /models/blocks/ and /textures/blocks/\n- 'item': Saves to /models/item/ and /textures/items/\n\n**Wiring behavior:**\n- By default, auto-discovers matching entity/block/item by modelId and wires the model\n- Set wireTo to a specific ID to wire to a different target\n- Set wireTo to false to skip wiring entirely\n\n**Iteration:** Running this tool again with the same modelId updates the existing files in place.",
        inputSchema: {
          projectPath: import_zod.z.string().describe("Absolute path to the Minecraft project folder (can be empty folder)"),
          design: mcpModelDesignSchema.describe("The model design specification"),
          modelId: import_zod.z.string().describe("Unique identifier for the model (e.g., 'disco_pig', 'magic_sword')"),
          usage: import_zod.z.enum(["entity", "block", "item"]).optional().describe("What the model is for - determines save location. Default: 'entity'"),
          wireTo: import_zod.z.union([import_zod.z.string(), import_zod.z.literal(false)]).optional().describe(
            "Wire the model to a specific entity/block/item ID, or false to skip wiring. Default: auto-discover matching item by modelId"
          )
        }
      },
      this._designModelOp
    );
    this._server.registerTool(
      "getModelTemplates",
      {
        title: "Get starter model templates for common Minecraft entity types",
        description: "Returns starter model designs for common Minecraft entity types. These templates provide proper Minecraft-scale geometry with blocky pixel-art textures. Use these as starting points and customize colors, proportions, and details. \n\nAvailable template types (all coordinates in Minecraft units where 16 units = 1 block):\n- 'humanoid': A humanoid mob with head, body, arms, legs (~32 units tall, ~2 blocks)\n- 'small_animal': A small quadruped like a pig or sheep (~14 units tall, ~1 block)\n- 'large_animal': A larger quadruped like a horse or cow (~24 units tall, ~1.5 blocks)\n- 'vehicle': A wheeled vehicle or machine (~48 units long, ~3 blocks)\n- 'block': A simple unit cube block (16x16x16 units = 1 block)\n- 'item': A handheld sword (~30 units tall, ~2 blocks)\n- 'bird': A bipedal bird with wings and beak (~12 units tall, ~0.75 blocks)\n- 'insect': An 8-legged spider/insect (~14 units tall, ~38 units wide with legs)\n- 'flying': A flying creature with articulated wings (~44 units wingspan, ~2.75 blocks)\n- 'fish': An aquatic fish with fins (~6 units long, ~0.4 blocks)\n- 'slime': A simple bouncy cube creature (~6 units, great for beginners)\n- 'wizard': A robed humanoid spellcaster with pointed hat (~40 units tall)\n- 'golem': A large bulky construct (~40 units tall, ~2.5 blocks)\n- 'fox': A sleek quadruped with fluffy tail (~12 units tall)\n- 'crystal': A geometric floating crystal formation (~20 units tall)\n- 'enchanted_sword': A glowing magical sword (~36 units tall)\n- 'tropical_fish': A colorful aquatic fish (~6 units)\n- 'ghost': A hovering spectral entity (~28 units tall)\n- 'robot': A mechanical humanoid (~36 units tall)\n- 'mushroom_creature': A fungal creature (~20 units tall)\n- 'treasure_chest': An openable container (~12 units tall)\n\n--- BLOCK TEMPLATES (16x16x16 = 1 block) ---\n- 'stone_brick': Classic masonry block with mortar lines\n- 'wooden_crate': Storage crate with metal corner brackets\n- 'glowing_ore': Luminescent mineral ore block\n- 'mossy_stone': Weathered stone with moss overlay\n- 'crystal_block': Translucent crystalline block\n- 'tech_block': Sci-fi mechanical block with vents\n\n--- ITEM TEMPLATES (handheld objects) ---\n- 'potion_bottle': Glass flask with cork stopper\n- 'magic_wand': Spellcasting wand with crystal tip\n- 'ornate_key': Decorative key with gem inlay\n- 'gemstone': Faceted precious stone\n- 'apple': Red apple with stem and leaf\n- 'pickaxe': Mining tool with iron head",
        inputSchema: {
          templateType: import_zod.z.enum([
            "humanoid",
            "small_animal",
            "large_animal",
            "vehicle",
            "block",
            "item",
            "bird",
            "insect",
            "flying",
            "fish",
            "slime",
            "wizard",
            "golem",
            "fox",
            "crystal",
            "enchanted_sword",
            "tropical_fish",
            "ghost",
            "robot",
            "mushroom_creature",
            "treasure_chest",
            "stone_brick",
            "wooden_crate",
            "glowing_ore",
            "mossy_stone",
            "crystal_block",
            "tech_block",
            "potion_bottle",
            "magic_wand",
            "ornate_key",
            "gemstone",
            "apple",
            "pickaxe"
          ]).describe("The type of template to return (entity, block, or item)")
        }
      },
      this._getModelTemplatesOp.bind(this)
    );
    const blockVolumeVector3Schema = import_zod.z.object({
      x: import_zod.z.number(),
      y: import_zod.z.number(),
      z: import_zod.z.number()
    });
    const blockTypeDataSchema = import_zod.z.object({
      typeId: import_zod.z.string().describe("Block type identifier, e.g., 'minecraft:stone' or 'stone'"),
      properties: import_zod.z.record(import_zod.z.union([import_zod.z.string(), import_zod.z.number(), import_zod.z.boolean()])).optional().describe("Optional block state properties, e.g., { facing: 'north', half: 'bottom' }")
    });
    const blockVolumeSchema = import_zod.z.object({
      entities: import_zod.z.array(
        import_zod.z.object({
          locationWithinVolume: blockVolumeVector3Schema,
          typeId: import_zod.z.string()
        })
      ).optional().describe("Entities within the structure (optional)"),
      southWestBottom: blockVolumeVector3Schema.describe(
        "The world position of the south-west-bottom corner of the structure"
      ),
      size: blockVolumeVector3Schema.optional().describe(
        "Optional dimensions (x=width, y=height, z=depth). If omitted, size is automatically inferred from the data."
      ),
      blockLayersBottomToTop: import_zod.z.array(import_zod.z.array(import_zod.z.string())).describe(
        "2D array of horizontal layers. Outer array is Y layers from BOTTOM to TOP (like stacking floors). Each layer is array of strings for Z rows from NORTH to SOUTH. Each character in string is X from WEST to EAST. Strings do NOT need to be the same length - shorter strings are treated as having trailing air blocks."
      ),
      key: import_zod.z.record(blockTypeDataSchema).describe("Map of single characters to block type definitions. Use ' ' (space) for air.")
    });
    this._server.registerTool(
      "designStructure",
      {
        title: "Design a structure in a Minecraft project",
        description: "Designs a structure (.mcstructure) and saves it to a Minecraft project folder. Use this tool for iterative structure design - it handles everything in one step:\n\n1. \u2705 Validates the block volume design\n2. \u2705 Creates the .mcstructure file\n3. \u2705 Saves to the correct project location (behavior_packs/<pack>/structures/)\n4. \u2705 Persists the design for future iteration (update existing structures)\n5. \u2705 Returns a preview image\n\n**Block Volume Format:**\nIBlockVolume represents a volume of blocks using horizontal layers (like stacking floors):\n- blockLayersBottomToTop: Array of Y layers from BOTTOM to TOP (ground floor first, roof last)\n- Each layer: Array of strings for Z rows from NORTH to SOUTH (back to front)\n- Each string character: X position from WEST to EAST (left to right)\n- key: Maps characters to block types\n\n**Flexible dimensions:**\n- 'size' is OPTIONAL - it will be automatically inferred from your data\n- Strings do NOT need to be the same length - shorter strings are treated as trailing air\n\n**Iteration:** Running this tool again with the same structureId updates the existing file in place.",
        inputSchema: {
          projectPath: import_zod.z.string().describe("Absolute path to the Minecraft project folder (can be empty folder)"),
          blockVolume: blockVolumeSchema.describe("The block volume design to build"),
          structureId: import_zod.z.string().describe("Unique identifier for the structure (e.g., 'castle_tower', 'dungeon_room')")
        }
      },
      this._designStructureOp
    );
    this._server.registerResource(
      "worldSession",
      new import_mcp.ResourceTemplate("worldSession://{name}", { list: void 0 }),
      {
        title: "World Session",
        // Display name for UI
        description: "Minecraft world session"
      },
      async (uri, name) => ({
        contents: [
          {
            uri: uri.href,
            text: `Hello, ${name}!`
          }
        ]
      })
    );
  }
  /**
   * Check if a port is available for use.
   * @param port The port number to check
   * @returns Promise that resolves to true if the port is available, false otherwise
   */
  static isPortAvailable(port) {
    return new Promise((resolve) => {
      const server = net.createServer();
      server.once("error", (err) => {
        if (err.code === "EADDRINUSE") {
          resolve(false);
        } else {
          resolve(false);
        }
      });
      server.once("listening", () => {
        server.close(() => {
          resolve(true);
        });
      });
      server.listen(port, "localhost");
    });
  }
  /**
   * Find an available port by randomly selecting from the configured range.
   * Excludes browser-unsafe ports that would cause ERR_UNSAFE_PORT errors.
   * @returns Promise that resolves to an available port, or a random port if none found after max attempts
   */
  async findAvailablePort() {
    const safePorts = [];
    for (let port = MinecraftMcpServer.PORT_RANGE_START; port <= MinecraftMcpServer.PORT_RANGE_END; port++) {
      if (!import_LocalUtilities.UNSAFE_PORTS.has(port)) {
        safePorts.push(port);
      }
    }
    const triedPorts = /* @__PURE__ */ new Set();
    for (let attempt = 0; attempt < MinecraftMcpServer.PORT_MAX_ATTEMPTS; attempt++) {
      let port;
      do {
        port = safePorts[Math.floor(Math.random() * safePorts.length)];
      } while (triedPorts.has(port) && triedPorts.size < safePorts.length);
      triedPorts.add(port);
      const available = await MinecraftMcpServer.isPortAvailable(port);
      if (available) {
        import_Log.default.verbose(`Found available port: ${port} (attempt ${attempt + 1})`);
        return port;
      }
      import_Log.default.verbose(`Port ${port} is in use, trying another random port...`);
    }
    const fallbackPort = safePorts[Math.floor(Math.random() * safePorts.length)];
    import_Log.default.debugAlert(
      `No available ports found after ${MinecraftMcpServer.PORT_MAX_ATTEMPTS} attempts, using ${fallbackPort}`
    );
    return fallbackPort;
  }
  async startStdio(creatorTools, env, workingFolder) {
    import_CreatorToolsHost.default.contentWebRoot = "https://mctools.dev/";
    this._creatorTools = creatorTools;
    this._env = env;
    this._workingFolder = workingFolder;
    this._creatorTools.onStatusAdded.subscribe(MinecraftMcpServer.handleStatusAdded);
    if (this._workingFolder) {
      import_Log.default.verbose(`MCP server working folder: ${this._workingFolder}`);
    }
    this._previewServerPort = await this.findAvailablePort();
    import_Log.default.verbose(`MCP server will use port ${this._previewServerPort} for internal HTTP server`);
    this._configureTools();
    this._configurePrompts();
    const transport = new import_stdio.StdioServerTransport();
    const cleanupAndExit = async () => {
      const forceExitTimeout = setTimeout(() => {
        import_Log.default.verbose("Force exiting after cleanup timeout");
        process.exit(0);
      }, 3e3);
      forceExitTimeout.unref();
      await this.cleanup();
      clearTimeout(forceExitTimeout);
      process.exit(0);
    };
    transport.onclose = cleanupAndExit;
    process.stdin.on("end", cleanupAndExit);
    process.stdin.on("close", cleanupAndExit);
    process.once("SIGINT", cleanupAndExit);
    process.once("SIGTERM", cleanupAndExit);
    this._server.connect(transport);
  }
  /**
   * Clean up resources (browser, HTTP server, etc.)
   */
  async cleanup() {
    if (this._cleaningUp) {
      return;
    }
    this._cleaningUp = true;
    if (this._cachedRenderer) {
      try {
        await this._cachedRenderer.close();
      } catch (e) {
      }
      this._cachedRenderer = void 0;
    }
    try {
      await import_ImageGenerationUtilities.default.closeCachedBrowser();
    } catch (e) {
    }
    if (this._serverManager) {
      this._serverManager.stopWebServer();
      this._serverManager = void 0;
    }
  }
  static handleStatusAdded(creatorTools, status) {
    let message = status.message;
    if (status.type === import_Status.StatusType.operationStarted) {
      message = "[[ START: (" + status.operationId + ") " + message;
    } else if (status.type === import_Status.StatusType.operationEndedComplete) {
      message = "          " + message + " (" + status.operationId + ")  :END ]]";
    } else if (status.type === import_Status.StatusType.operationEndedErrors) {
      message = "          " + message + " :END - ERRORS (" + status.operationId + ") ]]";
    }
    import_Log.default.message(message);
  }
  async startHttp(creatorTools, env) {
    this._creatorTools = creatorTools;
    this._env = env;
    this._configureTools();
  }
}
//# sourceMappingURL=MinecraftMcpServer.js.map
